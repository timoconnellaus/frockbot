...
/**
 * The `ctx` members that are always present: the identity of the call and the
 * three context keys. Nothing here is authority.
 */
...
/**
 * One `ctx` member per grant. A grant the member did not declare is never
...
export const BOT_ISOLATE_NARROW_CONTEXT_SOURCE_V1 = `function narrowContext(env, invocation) {
  ...
  const grants = env.IDENTITY.grants || [];
  const context = {
 ...
 */
export const BOT_ISOLATE_WRAPPER_SOURCE = `// Generated by @frockbot/compose-frockbot. Do not edit inside the isolate.
...
export const BOT_ISOLATE_WRAPPER_VERSION = "wrapper-v6";
