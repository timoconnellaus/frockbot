...
export function notificationIdV1(
  ...
  // Joined on a character no part can contain, so the digest separates inputs
  // the sanitiser would otherwise flatten together. The digest is always
  // appended, not only past the length ceiling: `("a:b")` and `("a-b")` both
  // sanitise to `a-b`, and without it one notification silently overwrote the
  // other.
  const raw = parts.map((part) => String(part)).join("\u0000");
  const sanitized = raw
    .replace(/[^a-zA-Z0-9._-]/g, "-")
    .replace(/^[^a-zA-Z0-9]+/, "");
  const seeded = sanitized.length === 0 ? "notification" : sanitized;
  const head = seeded.slice(
    0,
    MAX_NOTIFICATION_ID_LENGTH - DIGEST_SUFFIX_LENGTH,
  );
  const bounded = `${head}-${digest(raw)}`;
