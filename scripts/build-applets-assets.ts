...
const SKILL_SOURCE = "applets/skills/applets/";
...
const PLUGIN_SKILL_SOURCE = "app/plugins/skills/plugins/";
...
async function skillDirectory(
  ...
  const text = await Bun.file(at(`${directory}SKILL.md`)).text();
  const referencesDirectory = at(`${directory}references/`);
 ...
 *
 * It is authored in `applets/skills/applets/`, beside the feature it
...
async function skillModule(): Promise<string> {
  const skill = await skillDirectory(SKILL_SOURCE);
  return await prettyTypeScript(
    [
      "// Generated by scripts/build-applets-assets.ts. Do not edit.",
      "//",
      "// Authored at applets/skills/applets/.",
      DECODE_HELPER,
      'export const APPLETS_SKILL_SLUG_V1 = "applets";',
      `export const APPLETS_SKILL_DOCUMENT_V1 = fromBase64V1(${JSON.stringify(base64(skill.text))});`,
      `export const APPLETS_SKILL_REFERENCES_V1: ${REFERENCES_TYPE} = ${referencesLiteral(skill.references)};`,
      "",
    ].join("\n"),
  );
}
...
async function pluginSkillModule(): Promise<string> {
  const skill = await skillDirectory(PLUGIN_SKILL_SOURCE);
  return await prettyTypeScript(
    [
      "// Generated by scripts/build-applets-assets.ts. Do not edit.",
      "//",
      "// Authored at app/plugins/skills/plugins/.",
      DECODE_HELPER,
      'export const PLUGINS_SKILL_SLUG_V1 = "plugins";',
      `export const PLUGINS_SKILL_DOCUMENT_V1 = fromBase64V1(${JSON.stringify(base64(skill.text))});`,
      `export const PLUGINS_SKILL_REFERENCES_V1: ${REFERENCES_TYPE} = ${referencesLiteral(skill.references)};`,
      "",
    ].join("\n"),
  );
}
