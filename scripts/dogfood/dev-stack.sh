...
stop_stack() {
  ...
  # Backstop for a lost pid file, matched on this stack's own command line —
  # `--env development` on this port — so the end-to-end harness's
  # `wrangler dev --env e2e` on its own ephemeral port is never a match.
  for pattern in \
    "wrangler-dist/cli.js dev --env development --ip 127.0.0.1 --port $worker_port"; do
