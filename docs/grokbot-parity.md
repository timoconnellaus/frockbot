...
| 58  | **Beyond parity** — deployment-wide beta-access authority and owner-only administration                                                                                                       | not present in GrokBot                                                                                                                                             | FrockBot product   | `plugin-admin` + singleton `DeploymentPolicy` Durable Object                                                                                                                                                                                                                               | beyond parity                                       |
...
- **58** — **beyond parity.** Because the deployment's default model can incur a cost on every Turn, an identity is admitted only by the beta-access authority: a closed, invite-only or open admission mode, and an explicit per-account state that beats it ([`beta-access.md`](beta-access.md)). An existing identity or User is not access; deployment admins and local development identities remain admitted. The admin Contribution supplies the owner-only routes and page; the singleton `DeploymentPolicy` Durable Object owns the mode, the account records and the invitations under revisioned writes. This is FrockBot product policy, not a GrokBot capability being copied.
