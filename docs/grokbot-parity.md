...
| 35  | **Channels** — Bot-to-Bot group chats, 1–6 members, in the sidebar, posted into by id                                                                                                                                                                           | `CreateChannel`/`UpdateChannel`; `SendToAgent`                                                                                                                     | §2.14              | `app/groups` and the `GroupChat` Durable Object: the User and two to eight of their Bots in one thread, in the sidebar; a member answers when @mentioned or when Jev judges the message is theirs                                                                                          | diverged — Group Chats                              |
| 36  | External channel connectors (Telegram etc.) connected and disconnected per Bot                                                                                                                                                                                  | info-pane Channels; `update_state channel disconnect{platform}`                                                                                                    | §2.14, §2A         | `app/telegram`: one platform bot per deployment; each account links once with a one-time `/start` code and picks the Bot the chat talks to; the webhook admits user-lane `telegram` Turns; visible messages mirror through an at-most-once outbox.                                         | partial — Telegram 2026-09-24; email and Slack next |
...
| 54  | **Learn from demonstration** — a screen recording becomes a user-global skill, then the video is deleted                                                                                                                                                        | `learn-from-demonstration`: teach queue → `session.json` → `watchVideo` → `update_state skill write`                                                               | §3.9               | none — capture UI not started; managed set ships `write-skill` instead                                                                                                                                                                                                                     | not started                                         |
...
- **35** — **diverged — Group Chats.** The 2026-09-01 removal is reversed by Group Chats: a conversation between the User and two to eight of their Bots rather than GrokBot's Bot-only rooms of one to six ([architecture](architecture.md#groupchat--appscloudflaresrcgroup-chatts)).
- **36** — **partial — Telegram landed 2026-09-24, reversing the 2026-09-01 removal.** FrockBot runs one Telegram bot per deployment rather than a connector per Bot: a person links their Telegram account once from You → Telegram with a one-time code (`/start <code>`), picks which Bot the chat talks to there or with `/bot`, and their messages become ordinary user-lane Turns in that Bot's conversation, marked "via Telegram". Everything the Bot says in that conversation is mirrored back through an outbox beside push, at most once per message ([architecture](architecture.md#from-telegram)). Order from here: inbound email, then Slack; WhatsApp and SMS last. Not yet: attachments and voice notes from Telegram, group chats in Telegram, and a connector per Bot.
