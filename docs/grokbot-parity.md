...
| 36  | External channel connectors (Telegram etc.) connected and disconnected per Bot                                                                                                                                                                                  | info-pane Channels; `update_state channel disconnect{platform}`                                                                                                    | §2.14, §2A         | `app/email`: a per-Bot address, `<bot>.<username>@<domain>`, switched on per Bot; the sign-in address and confirmed senders become user-lane `email` Turns once DMARC passes.                                                                                                              | partial — inbound email 2026-09-24                  |
...
- **36** — **partial — inbound email landed 2026-09-24, reversing the 2026-09-01 removal for email.** Rather than a connector per Bot, each Bot has its own rotatable address, and a message from the person's sign-in address or an address they confirmed — by mailing back a code shown in the app — becomes a user-lane Turn in that Bot's conversation, marked "via email", with up to five files, once the receiving server's DMARC verdict proves the `From` domain sent it ([architecture](architecture.md#by-email)). Not yet: replies by email, which wait for outbound mail; other messaging apps; and a connector per Bot.
...
- **57e** — **email landed 2026-09-24.** The seeded `email` Plugin draws the draft as fields — To, Cc, Subject, the message — and the person's Send is a decision about what they left there: the card's `revise` restates the edit and the kernel moves the Approval's binding onto it in the transaction that records the decision ([ADR 0030](adr/0030-a2ui-cards.md), amended 2026-09-24), so `email_send` can send only their version. It sends through Cloudflare Email Service once a deployment profile names a sender (`scripts/deployment-config/README.md`); the hosted profile does not yet, so production drafts but does not send. Still missing, and why the row is partial: drafts for connected-app sends — a Gmail send, a Slack post through Composio still run the moment the Bot calls them — and GrokBot's per-platform composers.
