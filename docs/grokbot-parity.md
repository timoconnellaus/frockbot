...
`§4.<n>` cites `MESSAGE_4_RESPONSE.md` section `<n>`; `§6` cites the 2026-09-03 live progressive-tool probe in `MESSAGE_6_RESPONSE.md` and its attachments. Everything not marked **INFERENCE** is command output or a quote. GrokBot
...
| 10  | Three write scopes chosen per fact: agent, user, project                                                                                                                                                                                                        | `scope: agent\|user\|project`                                                                                                                                      | §2.2               | `plugin-memory`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | diverged — Group Chats, owner decision 2026-09-23   |
...
| 12  | Precedence: own > project > user-memory; newest write wins on conflict                                                                                                                                                                                          | injected: _"prefer your OWN memory first, then project memory, then user memory — the most specific wins"_                                                         | §2.3, §4.1a        | `plugin-memory`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | diverged — Group Chats, owner decision 2026-09-23   |
| 13  | Project memory tier: an opt-in third scope, per-Bot shards, only joined projects load, membership by create/join/leave                                                                                                                                          | `update_state project create\|join\|leave`; `projects/<slug>/memory/by-agent/<id>/` (**absent on disk**)                                                           | §2.2, §B5, §4.1a   | `plugin-memory`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | diverged — Group Chats, owner decision 2026-09-23   |
| 13b | An injection cap on the shared tiers: at most 3 projects, per-tier profile/recent limits and char budgets, per-fact clamp                                                                                                                                       | `MEMORY_PROJECT_INJECTED_CAP`=3; own `recall(30)`/4000/500; user 50/15; project 25/10                                                                              | §4.1b              | `plugin-memory`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | diverged — Group Chats, owner decision 2026-09-23   |
...
| 23  | **Computer** — one persistent Linux computer per user shared by all Bots, per-Bot durable roots, shared scratch                                                                                                                                                 | one container, 14 agents; `agent-data/agents/<uuid>/` vs `/workspace`                                                                                              | §B8, §A1           | `computer` + provider; `/workspace` scratch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | done                                                |
| 24  | Desktops allocated on demand, not one per Bot: own display, VNC/noVNC route, owner token, exec port                                                                                                                                                             | 7 live displays for 14 agents; `sand-window-router.mjs` `14000 + display`, `:1` primary                                                                            | §C12, §3.8         | `computer/fly`; viewer open/renew now touches the tenant's existing `last-seen`, so watching protects its allocated slot without a second activity marker                                                                                                                                                                                                                                                                                                                                                                         | partial                                             |
| 25  | Screenshot of the Bot's own desktop; human takeover for a login or captcha                                                                                                                                                                                      | native `Screenshot`; `request_box_help`                                                                                                                            | §16, §2A           | `computer_screenshot`; the hosted Computer card shows the newest durable capture without waking, expands one view-only noVNC session, and requires confirmation before switching that session to human control; human takeover holds the User-wide `desktop-gui` lease shared by every Bot                                                                                                                                                                                                                                        | landed                                              |
| 26  | "Update Computer" (fresh instance, keep files+logins, lose packages) and "Reset" (snapshot restore)                                                                                                                                                             | settings `update-computer` two-click confirm; `reset-computer`                                                                                                     | §2A, §9            | `computer`; in-place runtime updates stay automatic on wake. **Update Computer** replaces the machine and **Reset Computer** restores the newest checkpoint FrockBot recorded, each behind a confirm in the Bot's Computer settings that says what is kept (durable roots and sign-ins) and what is lost; **Save a checkpoint** beside them (`computer/bot.ts`, `apps/native/lib/computer/settings.dart`)                                                                                                                         | landed                                              |
| 26b | **Durability** — snapshot-out-only sync of the per-Bot state DBs on a debounce + tick, control files excluded                                                                                                                                                   | `box-store-sync`: 120 s tick, 5 s DB debounce, 15 min Chrome, `AGENT_STORE_DB_BASENAMES`, `box-store-sync.lock`                                                    | §3.3               | `computer`; the durable-root sync is the opposite mechanism (notes); the browser's cookies are carried off on a debounce (`computer/upkeep.ts`), no state-DB snapshot                                                                                                                                                                                                                                                                                                                                                             | partial                                             |
...
| 31  | **Browser** — one profile shared by all of a user's Bots                                                                                                                                                                                                        | `/home/box/chrome-profile`                                                                                                                                         | §B6                | `computer/fly`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | landed                                              |
| 32  | Cookies and logins survive computer replacement; seeding, periodic capture, cross-window mirroring, import                                                                                                                                                      | `sand-cookie-persist.mjs` (5 s → `chrome-cookie-seed.json`), `sand-session-sync.mjs` (1.5 s CDP), `chrome-cookie-import`                                           | §A3, §2A, §3.3     | `computer`; cookies captured over CDP at a human's release of the desktop, at most every 10 min at a browser Turn's end, and before an Update or Reset, sealed with the credential keyring in the User's object, restored into the machine that replaces or resets (`computer/upkeep.ts`, `app/shell/computer-logins.ts`); no cross-window mirroring, no import                                                                                                                                                                   | partial                                             |
...
| 36  | External channel connectors (Telegram etc.) connected and disconnected per Bot                                                                                                                                                                                  | info-pane Channels; `update_state channel disconnect{platform}`                                                                                                    | §2.14, §2A         | `app/email`: a per-Bot address, `<bot>.<username>@<domain>`, switched on per Bot; mail from the owner's addresses becomes user-lane `email` Turns once DMARC passes, and the Bot emails back from it.                                                                                                                                                                                                                                                                                                                             | partial — email 2026-09-24                          |
...
| 41  | **Connectors / MCP** — plugin = marketplace bundle of MCP connectors ± skills, stable numeric id                                                                                                                                                                | `plugins/cache/…/.cursor-plugin/plugin.json`                                                                                                                       | §2.10              | `app/connect` (the Marketplace's connector offers) + `app/mcp` (any remote MCP server a User adds by address); there is no bundle of connectors with skills and no numeric id                                                                                                                                                                                                                                                                                                                                                     | partial                                             |
| 42  | Plugin discovery, fetch, install state and uninstall over a 296-plugin catalog; plugins are user-scoped                                                                                                                                                         | `SearchPlugins`/`GetPlugin`/`UninstallPlugin`; `installed=yes (user)`                                                                                              | §2.10              | `plugin-settings` (the deployment catalog exposes its installable provider Plugins with account install state and uninstall; there is no third-party search or fetch)                                                                                                                                                                                                                                                                                                                                                             | partial                                             |
| 43  | Multi-account connectors with per-account labels; MCP lifecycle (status, restart, rename, instructions)                                                                                                                                                         | Gmail/Calendar/Drive ×5; `GetMcpServerStatus`, `SetMcpInstructions`                                                                                                | §16, §17           | `core/connection` + `app/connect` (several accounts of one app, each with its own label and namespace) + `app/mcp` (per-server status, rename, remove, refresh tools, the server's own instructions in the prompt)                                                                                                                                                                                                                                                                                                                | partial                                             |
| 44  | Custom (non-marketplace) stdio MCP servers, which cannot ship in a template                                                                                                                                                                                     | `user-beeper`, `user-beeper-desktop`                                                                                                                               | §2.10              | `app/mcp` holds custom remote servers by address (streamable HTTP and SSE); stdio is not built                                                                                                                                                                                                                                                                                                                                                                                                                                    | deferred                                            |
...
| 47  | Web search, web fetch and image generation as first-class tools                                                                                                                                                                                                 | `WebSearch`, `WebFetch`, `GenerateImage`                                                                                                                           | §17                | `plugin-web` + `providers/ollama-cloud` + `plugin-image`                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | landed                                              |
| 48  | **Registered machine** — registry of the user's own machines with live connected state                                                                                                                                                                          | `ListMachines` → `{machineId, label, connected}`                                                                                                                   | §2.16              | `plugin-user-machine`: User-DO registry, pairing/enroll/socket/claim/result routes, `machine_list`, desktop agent, settings section                                                                                                                                                                                                                                                                                                                                                                                               | landed                                              |
...
| 51  | Default Bot panel: live computer preview + Routines list; settings cog swaps the panel to Bot settings                                                                                                                                                          | right panel / settings cog                                                                                                                                         | §2A                | `plugin-settings` frame + the Bot page's Computer card / `plugin-routines` slot                                                                                                                                                                                                                                                                                                                                                                                                                                                   | landed                                              |
...
| 54  | **Learn from demonstration** — a screen recording becomes a user-global skill, then the video is deleted                                                                                                                                                        | `learn-from-demonstration`: teach queue → `session.json` → `watchVideo` → `update_state skill write`                                                               | §3.9               | `computer` Record in the viewer → CDP step log + masked screenshots → sent as a message's files → `write-skill` `demonstration.md` → approval → `skill_write` user → `demonstration_delete`; browser only                                                                                                                                                                                                                                                                                                                         | partial                                             |
| 55  | Guided connector install: catalog → setup fields → confirm widget → host-authored connect card → raw-MCP fallback                                                                                                                                               | `add-connector`: `SearchPlugins`/`GetPlugin`/`InstallPlugin{values}`/`AddMcpServer`/`SetMcpInstructions`                                                           | §3.10              | `connectors` + `add-connector`: `connectors_offer` checks the app against the catalog and draws a host-authored `ConnectApp` card; the raw-MCP fallback is the User adding the server by address in Connectors, which `add-connector` points to; no setup fields                                                                                                                                                                                                                                                                  | partial                                             |
...
| 57a | Progressive tool disclosure: native tools plus discovery/invocation meta-tools and a name-only prompt catalog                                                                                                                                                   | `GetDynamicTools`, `CallDynamicTool`, `<dynamic_tool_catalog>`; full schemas returned only by discovery                                                            | §6                 | `@frockbot/core/tools`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | landed                                              |
...
| 57d | Human takeover of the Bot's desktop for a login, captcha or payment                                                                                                                                                                                             | `request_box_help` (chat-only)                                                                                                                                     | §2A, §4.2          | `computer/fly` lease + WebUI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | not started                                         |
| 57e | Bot drafts an outbound external message; the user reviews and sends it from a composer card                                                                                                                                                                     | `DraftExternalMessage` (chat-only, also null for subagent runners)                                                                                                 | §4.2               | seeded `email` Plugin: an editable `draft` card whose `revise` rebinds the Approval to the person's edits, `email_send` over Cloudflare Email Service                                                                                                                                                                                                                                                                                                                                                                             | partial — email 2026-09-24                          |
| 57f | Bot-to-Bot messaging and emoji tapbacks as chat-turn tools; teammate/group directories arrive as prompt sections, not tools                                                                                                                                     | `SendToAgent`, `ReactToMessage`; `getAgentDirectorySection`/`getChannelsSection` (INFERENCE)                                                                       | §4.2               | `plugin-flock` `bot_message`; `agent` Turn/lane in `core/durable`; `<teammates>` and sender prompt sections; same-User User-DO lease; target transcript origin marker. Channels and tapbacks remain removed.                                                                                                                                                                                                                                                                                                                      | partial — direct Bot messages landed 2026-09-04     |
| 57g | Messages.app tools on the registered Mac behind a feature gate and a permission check                                                                                                                                                                           | `FindIMessageChats`/`ChatItems`/`SearchIMessages`/`IMessageActivity`/`FetchIMessageAttachment`/`SendIMessage`/`CheckIMessagePermissions`, `gates.messagesTools`    | §4.2               | first-party code removed; returns as a Plugin a Bot writes, on device modules ([ADR 0037](adr/0037-plugin-device-modules.md))                                                                                                                                                                                                                                                                                                                                                                                                     | removed                                             |
| 57h | Bot asks the user to authorize a one-time virtual payment card, gated on the payment connector                                                                                                                                                                  | `request_virtual_card`, `gates.stripeLink` + connector present                                                                                                     | §4.2               | groundwork only: `app/secrets` — a `secret-request` card collects a card number into the User's credential store and `computer_browser` fills it by reference under a fresh Approval bound to page, field and secret; no card issuing or payment connector (billing is not qualified for real payments)                                                                                                                                                                                                                           | not started                                         |
| 57i | Send pipeline: a user message sent mid-Turn reaches the running Turn — in GrokBot it interrupts it; in FrockBot it steers: the Turn yields at its next step boundary and the message runs next with that work in context, and background subagents keep running | `interrupt("superseded by a new user message")` + `enqueueExclusiveRun({lane:"user"})`; `interruptAll()` only for agent deletion; `unfinishedTasksReminderPending` | §B (send pipeline) | `plugin-shell` + kernel Turn admission: the user queue and `userMessageWaiting` at the loop's step boundary                                                                                                                                                                                                                                                                                                                                                                                                                       | diverged — steering, owner decision 2026-09-23      |
| 57j | The person attaches files to a message: pictures reach a model that can see them, documents arrive as their text, and the files stay with the Bot                                                                                                               | chat attachments; video attachments cap at ~15 MB (§3.9); `file_attachments` on `Task` (§2.15)                                                                     | §3.9, §2.15        | app `uploads` (upload route, per-Bot content-addressed store, account quota) + kernel `MessageAttachmentV1` on the Turn command, run record and `user/message`; bytes resolved per dispatch                                                                                                                                                                                                                                                                                                                                       | partial — first slice 2026-09-24                    |
| 57k | A secret the user types in chat — a password, a card number — is filled into the live browser and never reaches the agent                                                                                                                                       | `request_user_form` (`gates.userForm`): an in-chat form; the host fills the live browser, no values return to the agent                                            | §4.2               | `app/secrets` + `credentials` card + `computer_browser`: a `secret-request` send draws a host `SecretField` bound to a kernel-recorded request; the value is sealed as an account-scoped secret the Bot knows only by reference; a fill leases it for one CDP action on its own site, or under a fresh Approval for a payment detail or another site; listed and deleted in Settings · Saved secrets. One value per card, not a multi-field form; code the Bot runs on its Computer can read a filled field back (known issue 52) | partial                                             |
| 58  | **Beyond parity** — deployment-wide beta-access authority and owner-only administration                                                                                                                                                                         | not present in GrokBot                                                                                                                                             | FrockBot product   | admin Contribution + singleton `DeploymentPolicy` Durable Object                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | beyond parity                                       |
...
- **57j** — **first slice landed 2026-09-24.** The composer attaches up to five files of up to 20 MB — pictures (PNG, JPEG, WebP, GIF; HEIC converted and photos scaled to 2,048 pixels on the device), PDF, Word, Excel, PowerPoint, CSV and text or code. Each is uploaded to `POST /api/bots/:bot/uploads` before Send, admitted durably (R2 under the Bot's own prefix, a record in the Bot Durable Object, the account's 5 GB counted in the User object) and named in the Turn command by its SHA-256. A document is converted to text once, at upload (Workers AI `toMarkdown` with image description off, a small reader for PowerPoint and Word); the model sees up to 30,000 characters of it. Pictures go to the model as images, resolved after the request is journaled, so they survive an eviction and never enter the log; Frock AI Auto takes them on its gateway route, and `generate_image` now hands the Bot the picture it made. Deferred: a tool to read past a document's excerpt, the Computer reaching uploads, audio and video, and the share sheet.
- **58** — **beyond parity.** Because the deployment's default model can incur a cost on every Turn, account admission is governed by the [beta-access authority](beta-access.md). The admin Contribution supplies the owner-only routes and page. This is FrockBot product policy, not a GrokBot capability being copied.
...
- **10, 12, 13, 13b** — **diverged: Group Chats replaced Projects**
  ([plan](plan.md#planned-group-chats-replace-projects)). The third scope is
  a Group Chat's shared Memory rather than an opt-in Project: its members are
  its authority, a group's Turns inject it and the tools reach it with scope
  `group`, and there is no create, join or leave of its own. It is canonical
  Memory only, with no Markdown tier and so no 3-project cap. Precedence is
  own, then the group's, then the User's.
...
- **15** — landed. `RoutineRecordV1` in the Bot Durable Object
  ...
  prompt, schedule XOR webhook trigger, `enabled`, the writer of the creating
  and the latest write, the timestamps and `lastRunAt`; the User Profile owns
  the timezone, which is projected into each Bot for alarm evaluation;
  ...
  only as a digest.
- **16** — landed. Cron: five-field expressions evaluated by `croner` in the
  User Profile's IANA timezone; `CRON_TZ=` is refused so a Routine cannot
  override that account setting. The `@hourly`/`@daily`/`@weekly`/`@monthly`
  aliases and `@every <duration>` are validated at write time and evaluated by
  the scheduler (`plugin-routines/src/cron.ts`, tested across two daylight-saving
...
- **21** — all three halves are landed. **Managed** Skills are four
  ...
  ordered `bot` → `user` → `managed` → `plugin` under `SKILL_CATALOG_CAPS_V1`,
  every drop recorded as an `over-source-cap` refusal in `skill/injected`.
  **Plugin-borne** Skills returned with ADR 0030 step 2: a Plugin's descriptor
  carries `skills`, and `plugin/<pluginId>/<slug>` is the fourth rank, offered
  only to a Bot with that Plugin enabled. The
  **user-global** root
...
- **23** — one Computer per User with per-Bot durable roots is landed and
  ...
  no Sprites SDK. What keeps the row `partial` is no shared scratch, and row
  27 below.
- **24, 25** — a desktop slot, its viewer session, and the human-takeover lease
  ...
  capture — read through the authenticated Workspace route, so the picture is
  never an anonymous fetch — without opening a viewer or waking the Computer.
  Pressing the card, or the Computer icon in the bar, opens the shared
  noVNC session full window with `view_only=1`, which names the phase once as
  its app bar subtitle; **Take control**
  ...
  **Row 24 stays `partial`.** The allocator, the per-tenant display, the
  ...
  `computer/fly/computer.ts`), and an exec-only tenant holds its slot
...
- **28** — the interchangeable seam exists and has two implementations in
  tests, but only one is a Computer: `computer/host-protocol` is the
...
- **26** — **landed.** An Update is a fresh machine and a Reset is the newest
  checkpoint FrockBot recorded (one after every Update, whenever the User saves
  one, and weekly from a Turn's end); both keep the durable roots, through the
...
- **26b** — durability exists, by the opposite mechanism. The durable-root sync
  ...
  is no state-DB snapshot and no basename list. GrokBot's 15-minute Chrome
  snapshot has a counterpart now: the browser's cookies, captured at most every
  ten minutes at the end of a Turn that drove the browser and whenever a human
  hands the desktop back, kept sealed outside the Computer (row 32).
- **32** — **`partial`.** Sign-ins survive an Update and a Reset: the cookies,
  read and written over CDP (`browser.mjs` `logins-capture`/`logins-restore`),
  gzipped and sealed with the credential keyring by the Bot's object and kept by
  the User's, and put back into the machine that comes next — the debt an
  Update or Reset records is paid at its own end or at the next open, and no
  capture is kept while it is owed. GrokBot seeds a fresh box from a 5 s cookie
  snapshot; FrockBot captures at the moments a sign-in is made instead. Missing:
  cross-window session mirroring (`sand-session-sync`), importing cookies from
  the User's own browser (`chrome-cookie-import`), and everything in the
  profile that is not a cookie (known issue 57).
...
- **29** — landed. `computer_exec` grew a background mode that starts a command
  ...
  (`computer/fly/computer.ts`, "starts a command that outlives its
...
- **35** — **removed — owner decision 2026-09-01, not needed yet.** Bot-to-Bot group rooms, their UI, tools, durable records, and runtime path were removed from FrockBot.
- **36** — **partial — email landed 2026-09-24, reversing the 2026-09-01 removal for email.** Rather than a connector per Bot, each Bot has its own address, `<bot>.<username>@bots.frockbot.com`, switched on per Bot. A message from the person's sign-in address or an address they confirmed — by mailing back a code shown in the app — becomes a user-lane Turn in that Bot's conversation, marked "via email", with up to five files, once the receiving server's DMARC verdict proves the `From` domain sent it; and the Bot emails its person back from the same address, answering in the thread when the Turn came by email ([architecture](architecture.md#by-email)). Not yet: other messaging apps, a connector per Bot, and a third party's reply reaching the conversation.
- **57b/c** — A `card` payload carries one A2UI surface, folded into `shell:card:<surfaceId>` and answered through the Bot's cards route ([ADR 0030](adr/0030-a2ui-cards.md) step 3); the client draws it with `genui` from the catalogs it compiled in ([architecture.md, "Card — how a Bot renders"](architecture.md#card--how-a-bot-renders)). Each `send_to_user` call projects to its own visible message; plain assistant text stays private. The conversation contract defaults to short plain paragraphs, one thought per message, with separate sends for distinct parts of an answer. `continue` means more to say or do, and only the last message uses `finish`. Requested detail and structured output remain available. The development conversation evaluation covers these styles; the remaining payload and interaction gaps in issue #153 keep these rows partial.
- **57e** — **email landed 2026-09-24.** The seeded `email` Plugin draws the draft as fields — To, Cc, Subject, the message — and the person's Send is a decision about what they left there: the card's `revise` restates the edit and the kernel moves the Approval's binding onto it in the transaction that records the decision ([ADR 0030](adr/0030-a2ui-cards.md), amended 2026-09-24), so `email_send` can send only their version. It sends through Cloudflare Email Service from the Bot's own address, with replies going to the person (`scripts/deployment-config/README.md`); a note to the Bot's own person needs no draft (`email_owner`). Still missing, and why the row is partial: drafts for connected-app sends — a Gmail send, a Slack post through Composio still run the moment the Bot calls them — and GrokBot's per-platform composers.
- **57f** — **partially reinstated 2026-09-04 for issue #151.** Direct same-User `bot_message` calls now run a visible `agent` Turn and return the target Bot's `reply_to_request` answer to the asking Turn; both threads mark the exchange with GrokBot's "Messaged" / "Message from" line, which opens a view-only pair chat (2026-09-16). The teammate directory is a prompt section. Channels, group directories, emoji tapbacks, image attachments, and priority delivery remain removed or deferred.

- **41** — **partial.** The remote Package Catalog was deleted on
  2026-09-07, so there is no marketplace bundle and no `catalogId`: a Package
  is one of the deployment's own compiled-in Packages, named by `packageId`.
  The MCP half is back as app code: a User adds any remote MCP server by its
  address in Connectors (`app/mcp`), and its tools become one namespace on
  every Bot they own. The bundle half — connectors packaged with Skills under
  one id — has nothing to land in.
- **42** — **regressed 2026-09-07: the remote Package Catalog was deleted.**
  ...
  state.
- **43** — **partial.** The multi-account half is landed: a connected app
  holds several accounts, each its own Connection with its own label and
...
- **44** — **`deferred`, with the dependency stated.** A remote server is
  ...
  stdio needs all four of: (1) the computer-host slice —
...
- **47** — landed; all three tools exist. `web_search` and `web_fetch` are
  ...
  **No schema parity is claimed, and none is possible.** Row 47 cites §17,
  ...
  `app/web/ssrf.ts` is also the single outbound classifier `app/mcp` holds a
  User-named server address to, and every request and redirect hop after it.
...
- **39** — landed, at a different home than this register first guessed. The
  lease is not `computer/fly`'s: that one is per tenant layout, and the
