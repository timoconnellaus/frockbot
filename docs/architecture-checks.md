...
| a non-first-party Package loads with `globalOutbound` disabled and bindings derived only from what the Bot holds | `apps/cloudflare/test/bot-isolate.workerd.ts`                                                                                                                   | `a non-first-party Package loads with globalOutbound disabled and Bot authority bindings only`                                                                                                                                                                                            | workerd      |
| — a capability the Bot does not hold is unavailable                                                            | `apps/cloudflare/test/bot-isolate.workerd.ts`                                                                                                                   | `a capability the Bot does not hold is unavailable`                                                                                                                                                                                                                                       | workerd      |
| — adding or removing a Connection yields a new isolate                                                         | `apps/cloudflare/test/bot-isolate.workerd.ts`                                                                                                                   | `adding or removing a Connection yields a new isolate`                                                                                                                                                                                                                                    | workerd      |
| a Bot-authored UI page renders with no Package JavaScript in the app origin                                    | `packages/architecture-checks/src/package-ui-boundaries.test.ts`                                                                                           | `a Bot-authored UI page renders with no Package JavaScript in the app origin`                                                                                                                                                                                                             | Bun          |
| the bridge refuses a tool the Package did not declare                                                          | `packages/kernel-contracts/src/iframe-ui.test.ts`, `packages/plugin-shell/src/client/index.test.ts`, `packages/plugin-shell/src/backend-iframe-ui.test.ts` | `refuses a tool the Package did not declare`; `registers the sandbox settings host and refuses undeclared iframe tools before transport`; `refuses an undeclared tool before admitting a durable Turn`                                                                                    | Bun          |
| a page cannot read the app's cookies or storage                                                                | `packages/architecture-checks/src/package-ui-boundaries.test.ts`, `apps/cloudflare/test/ui-artifact.workerd.ts`                                            | `a page cannot read the app's cookies or storage`; `streams R2-backed HTML under the immutable CSP`                                                                                                                                                                                       | Bun, workerd |
...
| — an activation failure and quarantine are delivered as durable model-visible input on the fallback Turn       | `packages/plugin-shell/src/backend-composition-input.test.ts`                                                                                                   | `includes the authored Package, phase, diagnostics, and quarantine`                                                                                                                                                                                                                       | Bun          |
| every generation preserves the first-party bootstrap Package ids and provenance                                | `packages/kernel-do/src/composition-store.test.ts`                                                                                                              | `refuses proposals that remove or replace the first-party bootstrap core`                                                                                                                                                                                                                 | Bun          |
...
| — a Bot-callable undo records intent, appends an idempotent pending revert, and cannot mark it last-known-good | `packages/plugin-shell/src/backend-authoring.test.ts`, `packages/kernel-do/src/composition-store.test.ts`, `apps/cloudflare/test/authoring.workerd.ts`          | `package undo records intent before an idempotent Bot-origin revert without moving last-known-good`; `a Bot-origin revert is idempotent and cannot mark its generation last-known-good`; `package_undo records a Bot-origin pending revert and replay does not append another generation` | Bun, workerd |
| `package_inspect_self` reports the generated contract that the wrapper actually exposes                        | `packages/kernel-composition/src/isolate-wrapper.test.ts`, `apps/cloudflare/test/authoring.workerd.ts`, `apps/cloudflare/test/bot-isolate.workerd.ts`           | `the wrapper context keys equal the generated contract catalog`; `package_inspect_self returns the generated context and retained authored source`; `the runtime context keys equal the generated self-inspection catalog`                                                               | Bun, workerd |
| a Catalog install is pinned to the User's immutable generation and exact bundle hash                           | `packages/plugin-settings/src/user.test.ts`, `packages/plugin-shell/src/backend-package-catalog.test.ts`, `apps/cloudflare/test/package-catalog-bot.workerd.ts` | `admits only the exact hash of a code-carrying Catalog entry`; `records intent before the User effect, appends a summary generation, and replays`; `installs hash-pinned code, remounts after a Connection appears, and package_undo removes it`                                            | Bun, workerd |
| a Catalog Package with no ready declared Connection Type installs but remains inert, without a Bot prompt      | `packages/plugin-package-catalog/src/agent.test.ts`, `apps/cloudflare/test/package-catalog-bot.workerd.ts`                                                      | `records mutation intent before calling the host and logs the result`; `installs hash-pinned code, remounts after a Connection appears, and package_undo removes it`                                                                                                                      | Bun, workerd |
| undo of a Catalog install reconciles User installation state and appends an idempotent Bot-origin revert       | `packages/plugin-shell/src/backend-package-catalog.test.ts`, `apps/cloudflare/test/package-catalog-bot.workerd.ts`                                              | `package undo reverses an install in User state and appends a revert generation`; `installs hash-pinned code, remounts after a Connection appears, and package_undo removes it`                                                                                                           | Bun, workerd |
| a Bot cannot remove or replace a required first-party core member through the Catalog path                     | `packages/plugin-shell/src/backend-package-catalog.test.ts`, `packages/kernel-do/src/composition-store.test.ts`                                                 | `hash mismatch and required-core removal are refused before User state changes`; `refuses proposals that remove or replace the first-party bootstrap core`                                                                                                                                | Bun          |
...
| a Bot receives Project authority only through durable membership: an unjoined Project is refused           | `packages/plugin-memory/src/agent.test.ts` | `is refused, and nothing is recorded or written`                               | Bun    |
...
| S6 `/app-manifest` producer → the client's plugin-catalog decode | 2 and 3 — the decoder refused the producer's manifest and the panel banner lit | `apps/cloudflare/e2e/bot-settings.e2e.ts`   | The gear opens Bot settings with the model catalog decoded and no error banner anywhere                             |
