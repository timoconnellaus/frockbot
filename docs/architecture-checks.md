...
Rows for `docs/plans/computer-presence.md` Step 1: the Bot Durable Object is
...
| local and hosted Computer clients share every card phase transition, including error and retry                       | `packages/plugin-computer/src/client/state-machine.test.ts` | `moves through every viewer and control transition used by the card`, `moves to error without discarding whether control was held`, `retry returns an error state to provisioning` | Bun     |
...
| the hosted client submits versioned commands through the authenticated hosted transport                              | `packages/plugin-computer/src/client/application.test.ts`   | `the hosted Computer provider drives the card through a fake transport`                                                                                                            | Bun     |
