...
| a Catalog install is pinned to the User's immutable generation and exact bundle hash                           | `packages/plugin-settings/src/user.test.ts`, `packages/plugin-shell/src/backend-package-catalog.test.ts`, `apps/cloudflare/test/package-catalog-bot.workerd.ts` | `admits only the exact hash of a code-carrying Catalog entry`; `records intent before the User effect, appends a summary generation, and replays`; `installs hash-pinned code, activates it next Turn, and package_undo removes it`                                                       | Bun, workerd |
| a Catalog Package with no ready declared Connection Type installs but remains inert, without a Bot prompt      | `packages/plugin-package-catalog/src/agent.test.ts`, `apps/cloudflare/test/package-catalog-bot.workerd.ts`                                                      | `records mutation intent before calling the host and logs the result`; `installs hash-pinned code, activates it next Turn, and package_undo removes it`                                                                                                                                   | Bun, workerd |
| undo of a Catalog install reconciles User installation state and appends an idempotent Bot-origin revert       | `packages/plugin-shell/src/backend-package-catalog.test.ts`, `apps/cloudflare/test/package-catalog-bot.workerd.ts`                                              | `package undo reverses an install in User state and appends a revert generation`; `installs hash-pinned code, activates it next Turn, and package_undo removes it`                                                                                                                        | Bun, workerd |
