...
| — its bindings derive only from enabled Capabilities                                                                            | `apps/cloudflare/test/bot-isolate.workerd.ts`                       | `list reports the User-enabled capability set`                                                                                                              | workerd |
...
| S6 `/app-manifest` producer → the client plugin-catalog decode  | 2 and 3 — the decoder refused the producer manifest and the panel banner lit | `apps/cloudflare/e2e/bot-settings.e2e.ts`   | The gear opens Bot settings with the enabled Capability catalog decoded and no error banner anywhere                |
