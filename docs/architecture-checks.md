...
| a non-first-party Package loads with `globalOutbound` disabled and User-enabled bindings                                        | `apps/cloudflare/test/bot-isolate.workerd.ts`                       | `a non-first-party Package loads with globalOutbound disabled and User-enabled bindings only`                                                       | workerd |
| — `CAPABILITIES.list()` projects the complete User-enabled set                                                                  | `packages/plugin-shell/src/backend-isolate.test.ts`                 | `list projects the complete enabled set`                                                                                                            | Bun     |
| — the binding digest is Bot-independent and changes when the User-enabled grant changes                                         | `packages/plugin-shell/src/backend-isolate.test.ts`                 | `two Bots share a digest and a changed grant changes it`                                                                                            | Bun     |
...
| the hosted client submits versioned commands through the authenticated hosted transport                                       | `packages/plugin-computer/src/client/application.test.ts`                                             | `mounts the card and strip without connecting the Computer`                                                                                                                                                   | Bun     |
...
| the sidebar mounts from the Computer Package, reads only durable projection state, and does not connect on mount              | `packages/plugin-computer/src/client/application.test.ts`                                             | `mounts the card and strip without connecting the Computer`                                                                                                                                                   | Bun     |
...
| the durable screenshot is rendered again only when its content hash changes                                                   | `packages/plugin-computer/src/client/ComputerStrip.test.ts`                                           | `a repeated screenshot projection preserves the rendered capture`                                                                                                                                             | Bun     |
...
| the shared client machine enters and leaves `updating`; an updating strip click rejoins the update and every poll while open asks again until the host is ready | `packages/plugin-computer/src/client/state-machine.test.ts`, `packages/plugin-computer/src/client/application.test.ts` | `moves into updating with its label and out when connection completes`, `an updating strip click rejoins the update and lands on ready when it finishes` | Bun    |
