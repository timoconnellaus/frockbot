...
4. Keep summary bytes unchanged until a committed compaction. Compaction may read its covered archive independently in the background. Commit summary/coverage and context revision together, checking the expected epoch and covered tail. Preserve the single Session writer when new input arrives during a detached compaction: it keeps summarising, and an outcome that lands while a Turn holds the log is parked and written at the next Turn end.
...
4. The single driver reconciles the previous active run before promoting work. Honor durable Stop and uncertain paid outcomes. Never execute two Turns concurrently in one conversation merely to make admission faster.
...
Extend Turn admission/queue/retry tests, workerd admission tests and any affected internal caller tests. Hold the old provider call unresolved and prove the new command is acknowledged durably. Evict after admission before the in-memory kick; the alarm must continue the recorded work. Exercise Stop and steering races, retries with the same command, background/agent lanes, queued artifact changes, exact pending-request recovery and one active Session writer.
