...
`@frockbot/plugin-fly-sprite` is the first built-in Computer package. One persistent Sprite provides a shared `/home/box` and `/workspace`, while a deep bot binding maps explicit agent identity to a deterministic data directory, Chromium profile, X display, CDP port, VNC port, and `flock`-serialized takeover lease. Its agent contribution consumes `ToolExecutionContext.agentId`, exposes guarded shell and accessibility-driven Chromium tools, recalls standing memory from the bot and user profile files, and mirrors completed session journals. Its desktop contribution binds to `FROCKBOT_AGENT_ID`; its trusted WebUI contribution embeds the corresponding viewer in `frockbot.computer`. A single websockify `TokenFile` gateway routes opaque viewer tokens to bot-scoped loopback VNC ports. The Sprites token remains process-local. Human takeover blocks new computer calls only for the leased bot, so other bot desktops remain available. These per-bot namespaces share one Unix account and are coordination boundaries rather than security isolation. Heartbeat loss re-shields the viewer, and expired leases can be atomically reclaimed after a crash; the lease remains a coordination mechanism rather than a security boundary for work already in flight.
