...
Six Workers, two container images, one Flutter client — on the web and on the phone.
...
- `.github/workflows/release.yml` — on tag `v*.*.*`, deploys marketing and the admin portal in one job, the computer host, the Applet build service and the app Worker. The portal's step skips when the `production` environment names no Access application.
