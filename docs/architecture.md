...
- The conversation prompt defaults to plain paragraphs, one thought and usually one to three short sentences per message. Simple answers use one message; distinct parts can use two to four, keeping the whole reply concise. Headings, bold labels, lists and tables are reserved for requested structured output; links and necessary code remain available, and a request for detail gets it.
