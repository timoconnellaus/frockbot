...
| `MEMORY_INDEX` (:78)                                     | Vectorize          | Memory embeddings: upserted and deleted by the Memory drain (`app/memory/processing.ts`), deleted by the Bot's vector purge (`bot-state.ts:1225-1260`), queried for semantic recall (`bot-state.ts:1148-1160`, `app/memory/semantic.ts`) |
