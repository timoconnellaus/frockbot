...
The renderer is `apps/native/lib/cards/`: `client.dart` (the three routes — list, read one, post one press), `surface.dart` (admission and the translation), `catalog.dart` (the catalogs, as one), `press.dart` (which press the card is waiting on), `chat_card.dart` (`CardChatCard`, the widget in the transcript) and `frock_catalog/` (the Frock components and their schemas).
