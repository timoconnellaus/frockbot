...
**What is still missing.** There is no webhook route and no key store, so a webhook Routine can only be fired by hand; and an automation Turn is not yet separated from the visible transcript — it appears in `GET /api/bots/:id/turns` today, and there is no completion inbox and no pending wake. Those are the next PRs.
