...
- Holds each Bot's inbound email address (`email:address:<botId>`) and the account's confirmed and waiting senders (`email:senders`), and decides every message the `email()` handler lets through (`routeInboundEmail`, §3 "By email").
...
- Also keeps the inbound email directory (`app/email/directory.ts`): a message names only its recipient's token, and the whole deployment's one shared object is what can say whose it is before any User's object is addressed. The digest of each Bot's token maps to its User and Bot here (`email:address:v1:<digest>`, and `email:bot:v1:<userId>/<botId>` the other way round). `forgetAccount` forgets them too.
...
**Addresses and senders.** Bot settings → Email (`apps/native/lib/email/page.dart`) reads `GET /api/bots/:botId/email` and changes it with `POST …/email/address` (`create`, `rotate`, `remove`) and `POST …/email/senders` (`add`, `remove`); each answers the whole view. A Bot's address is `<token>@<domain>`, the token 26 random base32 characters. The User object holds it and registers its SHA-256 in the `DeploymentPolicy` directory before keeping it, under `blockConcurrencyWhile` so two rotations cannot interleave the two writes; rotating registers the new digest and drops the old one in one directory write, and removing the address or deleting the Bot releases it. Who may write is the account's, one list for every Bot: the sign-in address when the identity provider verified it (read from the stored identity, never from a session), and up to ten more the person added. An added address waits with a code shown on the page, `FROCK-XXXX-XXXX`, for a day; it is confirmed when a message from it carrying that code reaches any of the User's addresses with a passing DMARC verdict — a code shown in the app and sent back from the mailbox, never a link sent to it, which there is no outbound mail to send anyway.

**Inbound** (`inbound.ts`). The order is the point, because anyone can send anything naming anyone in `From`. With nothing asked of any object: a message over 20 MB is refused (`rawSize`, then again while the stream is read); the recipient must be a well-formed token at the inbound domain (`token+anything` is the same address); a bounce (null envelope sender) or an `Auto-Submitted` message is refused; the MIME is parsed (`postal-mime`, bundled); `From` must hold exactly one mailbox in exactly one header; and the sender must be authenticated (`authentication.ts`). Only the topmost `Authentication-Results` or `ARC-Authentication-Results` header is believed, because a receiving server adds its own above everything it received, and only when its authserv-id is the receiving server's own, `mx.cloudflare.net`. It passes when DMARC passed for the `From` domain, or — for a domain that publishes no DMARC policy — when that domain itself signed with DKIM; anything else, including no verdict at all, is refused. A message must also carry a usable Message-ID. Then the directory names the User and Bot; the account admission every external door asks (`externalAccountRefusal`); and the User object (`routeInboundEmail`) checks the token against the Bot's current address, that the Bot is active, and that the sender is the sign-in address or a confirmed one — or confirms a waiting address whose code the message carries, which makes no Turn. Every refusal is `setReject`, a permanent SMTP error the sending server reports to its own sender, so a forged `From` is never written back to; the text says what happened and nothing about whose address it is.
...
`email.integration.ts` hands messages to the deployed Worker's own `email()`: the owner's message with two files admitted once however often it arrives, a failed DMARC verdict, a stranger, an unknown token and an oversized message refused, an added address confirmed by its code, a rotated address refused, and a deleted account's addresses gone from the directory.
