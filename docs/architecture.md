...
- The grants reach the Bot object as `isolateAuthority`, `isolateInvokeModel` (the Bot injects the model binding it resolved; a Plugin names a provider and a model, never a Connection), Memory, Workspace, `isolateConnection`, `isolateSchedule`, `isolateStorage{Get,Put,Delete,List}` (a key-value store under `plugin:storage:<pluginId>:` in the Bot object, bounded per value and per Plugin) and `isolateSettings` (`plugin:settings:<pluginId>`, the values the User set for this Bot).
