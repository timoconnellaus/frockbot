...
- Binding `VOICE_ASSISTANTS`; one object per User, reached with `get(idFromName(userId))` through the gateway's `GET /api/voice/assistant` upgrade (there is no `/agents/*` route). The upgrade is not routed through `getAgentByName`, which would wait for `onStart` before returning the 101. Dictation leases still use `getAgentByName` because they are RPC. Migration `v7`, `new_sqlite_classes`.
