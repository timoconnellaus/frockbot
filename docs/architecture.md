...
`@frockbot/plugin-composio` is the first Connection Package. FrockBot records a durable Connection intent before asking Composio for a hosted Connect Link and uses the Connection ID as the provider alias so a lost response can be reconciled by a read instead of repeating the effect. Composio retains and refreshes external OAuth credentials; FrockBot keeps only safe metadata and opaque connected-account identifiers. Callback transitions are conditional Durable Object transactions: provider verification first claims an absolute-expiry Bot-assignment lease, a lease-specific idempotent Bot command writes or restores the Assignment, and only then can the Connection become ready. Concurrent callbacks cannot share a lease. Expiry durably records lease-scoped compensation before an alarm disables any possibly stale Assignment. Both the Bot mutation and the User-owned acknowledgment carry an immutable compensation ID plus the expected assignment generation, so stale work can neither disable a newer generation nor clear newer compensation. A targeted legacy Assignment without generation metadata uses a revocation-only wildcard after the Connection has entered revoking, where no newer assignment can be admitted. Failed compensation retains an absolute retry deadline and blocks reassignment until repaired. Revocation similarly records assignment compensation for every targeted Bot, claims the Connection, records provider completion, retries failed invalidation by alarm, and automatically finalizes revoked only after the matching compensation is acknowledged. Alarms always schedule the earliest stored deadline, while retries preserve which operation needs reconciliation; uncertain provider outcomes are inspected but never automatically repeated. The hosted browser exposes this flow; mobile hides the Plugins control until its native OAuth/deep-link return adapter is implemented, rather than presenting a connection flow that cannot complete with bearer-token authentication.
