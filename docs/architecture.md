...
- Holds which Bots receive email (`email:receiving:<botId>`) and the account's confirmed and waiting senders (`email:senders`), and decides every message the `email()` handler lets through (`routeInboundEmail`, §3 "By email").
...
A person may also email a Bot (`app/email/`). The Worker exports `email()` beside `fetch()`: Cloudflare Email Routing hands it every message sent to the deployment's inbound domain, named by the deployment profile's optional `inboundEmail.domain` and set as the `INBOUND_EMAIL_DOMAIN` var (`scripts/deployment-config/README.md` has the routing setup). Absent, every message is refused and the Bot's Email page says email is not set up.

**Addresses and senders.** A Bot's address is `<bot-slug>.<username>@<domain>`, `fox.tim@frockbot.com`. The username is the account's, chosen under Account → Email username (`apps/native/lib/email/username.dart`, `GET`/`POST /api/email/username`, `{username}` or `null` to give it up): 3 to 30 characters of `[a-z0-9-]`, starting with a letter, with no leading, trailing or double dash, outside a reserved list (`RESERVED_EMAIL_USERNAMES_V1`: `postmaster`, `abuse`, `noreply` and the like), and held by one account across the deployment. Changing it changes every Bot's address at once, and the page says so before it saves; the old name is anyone's. The slug is the Bot's current name (`botEmailSlugsV1`, `app/email/shared.ts`): lowercased, whitespace, `_` and `.` as dashes, accents folded (`é` → `e`, `ß` → `ss`), anything else outside `[a-z0-9-]` dropped, dashes collapsed and trimmed, at most 32 characters; a name that leaves nothing is `bot-` and the first six of the Bot's id; two Bots that come out the same are told apart by creation order, the later `-2`, then `-3`. It is derived from the Flock directory on every read, never stored, so renaming a Bot changes its address and the old one is simply no Bot's. Bot settings → Email (`apps/native/lib/email/page.dart`) reads `GET /api/bots/:botId/email` and changes it with `POST …/email/switch` (`{receiving}`; off until the person turns it on, and only an active Bot turns on) and `POST …/email/senders` (`add`, `remove`); each answers the whole view, with the address and a Copy button, or — with no username yet — a line saying so and a button to the username page. Who may write is the account's, one list for every Bot: the sign-in address when the identity provider verified it (read from the stored identity, never from a session), and up to ten more the person added. An added address waits with a code shown on the page, `FROCK-XXXX-XXXX`, for a day; it is confirmed when a message from it carrying that code reaches any of the User's addresses with a passing DMARC verdict — a code shown in the app and sent back from the mailbox, never a link sent to it, which there is no outbound mail to send anyway.
...
Top-level vars: `NATIVE_SLICE_2_AUTH`, `DEFAULT_APPLICATION_HASH`, `FROCK_AI_GATEWAY_ID`, `FROCK_AI_ACCOUNT_ID`, `FROCK_AI_AUTO_ROUTE`, and `INBOUND_EMAIL_DOMAIN` where the profile names `inboundEmail` (§3, "By email"). `ALLOWED_CLIENT_ORIGINS` is read but set nowhere: the web app is same-origin and the Flutter app sends no `Origin`.
