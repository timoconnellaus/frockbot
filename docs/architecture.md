...
**Staying put.** The card reads over REST when it mounts and re-reads when the Bot's durable state is invalidated (`ChatController.invalidations`, bumped once per state-channel notice), and is kept alive in the transcript like an Applet card. A notice names no record, so most re-reads come back at the revision already drawn; every fold bumps the revision, so such a record is the record already drawn and keeps the renderer it is in, which is where what the person has typed lives. A record that did move gets a fresh controller, and adopting any record ends the reads older than it, so a read still in flight never redraws over the receipt a press carried back. A later `card` send naming the same `surfaceId` updates the record rather than adding a second card, so `dedupeCardSendsV1` (`lib/shell/transcript_model.dart`) drops the repeat where the thread is ordered and the card stays where it was first drawn.
