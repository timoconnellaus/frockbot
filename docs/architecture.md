...
- `POST /api/bots/:bot/secret-requests/:requestId` is a value a person typed into a secret request's `SecretField` — `{schemaVersion, commandId, value}` — and is never written here. It is checked against the `secret-request:<requestId>` record the card's draw wrote, handed to the User object's `storeSecret` to seal, and then, in one transaction, the request is marked answered, the field is folded to `state: "saved"` and a `secret-saved` pending input carrying the reference is queued; that input opens an input-delivery Turn. A request already answered reads back and saves nothing (§10, [Saved secrets](#saved-secrets--appsecrets)).
