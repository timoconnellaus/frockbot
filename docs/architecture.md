...
**The `tuner` Plugin.** Seeded `default-off`, with no tool and no card: one `conversation.panel` view whose `page` is `tuner.html`, and the `device` grant for the microphone. It is the tuner `references/microphone.md` teaches, file for file, under the deployment's id (`tuner-example.test.ts` holds them equal). Switching it on is the approval — the Plugins row says the page can use the microphone before the switch is flipped — and each use is still the host's: the bar, the Stop, the page told why. A seeded page is built with the bridge injected exactly as a publish stores one and travels in the bundle beside the module (`pages` in `artifacts.generated.ts`), so the page route answers a seeded hash from the bundle before it asks R2, and reconciliation re-ships an account whose member carries an older page.
