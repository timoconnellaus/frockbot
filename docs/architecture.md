...
- Its constructor runs receipted, disposable cleanups under `blockConcurrencyWhile` before any request or alarm: Plugin-panels (`plugin-panels-cleanup.ts`) drops directory entries, Composition `applets[]`, the account `applets` flag and cleanup to-dos; `cleanDefaultPackagesMarkerV1` (`default-packages-marker-cleanup.ts`) deletes pre-ledger `{ schemaVersion: 1 | 2 | 3 }` bootstrap markers under `maintenance:default-packages-marker:2026-09-20`; the default-Package bootstrap decoder accepts only the v4 ledger. Package-page shapes (`package-page-shapes-cleanup.ts`, `maintenance:package-page-shapes:2026-09-24`) runs before anything decodes a run or a Session event: a finished run loses its `directTool` field, an unfinished one is removed with its active, queue, index and repair pointers, and a `computer/sync` whose `reason` was `publish` reads `signal`. Bot templates (`bot-template-cleanup.ts`, `maintenance:bot-template-removal:2026-09-25`) deletes the `templates/<hash>.json` artifact each share record names, every `bot-template:` key and the account's `bot-template` Package row.
...
- Holds the User's browser sign-ins, sealed, under `computer:browser-logins:v1`: one capture, the debt an Update or a Reset leaves, and when the User last deleted the Computer (§10, "Update, Reset and the User's sign-ins"). It stores the envelope a Bot's object sealed and never holds the key. "Delete my Computer" forgets the capture and the debt on both sides of the teardown.
...
- Holds the User's saved secrets (`app/secrets/user.ts`): a `secret:item:<secretId>` record — name, site, payment class, the Bot that asked — beside a credential generation sealed under the `secrets` Package, one per request (`secret:request:<botId>:<requestId>`). `storeSecret`, `describeSecret`, `leaseSecret` and `settleSecret` are the Bot's RPCs; `GET /api/secrets` (`?as=document` for Settings) and `POST /api/secrets/:secretId/delete` are the person's. Nothing answers a value.
- Holds which Bots have email switched on (`email:enabled:<botId>`) and the account's confirmed and waiting senders (`email:senders`), decides every message the `email()` handler lets through (`routeInboundEmail`), and answers what a Bot sends as (`readBotEmailSender`, §3 "By email").
...
- Also keeps the email usernames (`app/email/directory.ts`): a message names a username, and the whole deployment's one shared object is what can say whose it is before any User's object is addressed, and keep each to one account. `email:username:v1:<username>` names the User, and `email:user:v1:<userId>` the username the other way round; `claimEmailUsername` checks and writes both in one synchronous transaction and releases the User's previous name in the same write. `forgetAccount` releases it too.
...
A person may also email a Bot, and a Bot may email back (`app/email/`). Both directions use one domain, the deployment profile's optional `email.domain`, set as the `EMAIL_DOMAIN` var — `bots.frockbot.com` for `hosted` (`scripts/deployment-config/README.md` has the routing and sending setup). The Worker exports `email()` beside `fetch()`: Cloudflare Email Routing hands it every message sent to that domain. Absent, every message is refused, nothing is sent, and the Bot's Email page says email is not set up.

**Addresses and senders.** A Bot's address is `<bot-slug>.<username>@<domain>`, `fox.tim@bots.frockbot.com`, and it is where mail to the Bot arrives and where mail from it leaves. The username is the account's, chosen under Account → Email username (`apps/native/lib/email/username.dart`, `GET`/`POST /api/email/username`, `{username}` or `null` to give it up): 3 to 30 characters of `[a-z0-9-]`, starting with a letter, with no leading, trailing or double dash, outside a reserved list (`RESERVED_EMAIL_USERNAMES_V1`: `postmaster`, `abuse`, `noreply` and the like), and held by one account across the deployment. Changing it changes every Bot's address at once, and the page says so before it saves; the old name is anyone's. The slug is the Bot's current name (`botEmailSlugsV1`, `app/email/shared.ts`): lowercased, whitespace, `_` and `.` as dashes, accents folded (`é` → `e`, `ß` → `ss`), anything else outside `[a-z0-9-]` dropped, dashes collapsed and trimmed, at most 32 characters; a name that leaves nothing is `bot-` and the first six of the Bot's id; two Bots that come out the same are told apart by creation order, the later `-2`, then `-3`. It is derived from the Flock directory on every read, never stored, so renaming a Bot changes its address and the old one is simply no Bot's. Bot settings → Email (`apps/native/lib/email/page.dart`) reads `GET /api/bots/:botId/email` and changes it with `POST …/email/switch` (`{enabled}`, the page's **Email** switch: receiving and sending both; off until the person turns it on, and only an active Bot turns on) and `POST …/email/senders` (`add`, `remove`); each answers the whole view, with the address and a Copy button, or — with no username yet — a line saying so and a button to the username page. Who may write is the account's, one list for every Bot: the sign-in address when the identity provider verified it (read from the stored identity, never from a session), and up to ten more the person added. An added address waits with a code shown on the page, `FROCK-XXXX-XXXX`, for a day; it is confirmed when a message from it carrying that code reaches any of the User's Bots' addresses with a passing DMARC verdict — a code shown in the app and sent back from the mailbox, never a link sent to it.

**Inbound** (`inbound.ts`). The order is the point, because anyone can send anything naming anyone in `From`. With nothing asked of any object: a message over 20 MB is refused (`rawSize`, then again while the stream is read); the recipient must be `<slug>.<username>` at the inbound domain, split at its last dot, neither half holding one (`+anything` is the same address; a local part with no dot is never a Bot's, so the domain may be the apex and its other mailboxes are none of this handler's); a bounce (null envelope sender) or an `Auto-Submitted` message is refused; the MIME is parsed (`postal-mime`, bundled); `From` must hold exactly one mailbox in exactly one header; and the sender must be authenticated (`authentication.ts`). Only the topmost `Authentication-Results` or `ARC-Authentication-Results` header is believed, because a receiving server adds its own above everything it received, and only when its authserv-id is the receiving server's own, `mx.cloudflare.net`. It passes when DMARC passed for the `From` domain, or — for a domain that publishes no DMARC policy — when that domain itself signed with DKIM; anything else, including no verdict at all, is refused. Addresses are guessable by design, a Bot's name and a username, so this check with the sender allowlist behind it is the only lock on a Bot's inbox; it is never relaxed. A message must also carry a usable Message-ID. Then the directory names the User from the username; the account admission every external door asks (`externalAccountRefusal`); and the User object (`routeInboundEmail`) checks first that the sender is the sign-in address or a confirmed one — or confirms a waiting address whose code the message carries, which makes no Turn — and only then which of the User's Bots the slug names, that it is active, and that its switch is on. A stranger is refused before any Bot is looked at, so a guessed address says nothing about which Bots there are. Every refusal is `setReject`, a permanent SMTP error the sending server reports to its own sender, so a forged `From` is never written back to; the text says what happened and nothing about whose address it is.

What the Bot is given: `Subject: …`, then the body with a reply's quoted history cut at the line that introduces it and a signature cut at `-- ` or a phone's footer (a forward is kept whole; an HTML-only body is read in one linear pass, since it is parsed before the sender is known), bounded to 16,000 characters and 30,000 bytes, and up to five files through the same store as the composer's uploads (`storeUploadV1`, `apps/cloudflare/src/uploads.ts`: the same types, 20 MB each, the account's quota). Attachments come first, then pictures placed in the body; an inline part under 20 KB is a signature's logo and is skipped. A file refused, or past the fifth, is named in a `Not attached:` line so the Bot can say so. The Bot admits it with `admitEmailTurn`: an ordinary `chat` Turn on the user lane in the Bot's own Session with origin `{kind: "email", messageId, quiet?}` and run id `em-<sha256(recipient, Message-ID)>`, so a redelivery replays the admission; the files are refs to uploads the handler recorded on that Bot first. A full queue and a Bot archived or deleted in between are refused; a failure to reach durable state throws, and Email Routing delivers the message again. The thread projects the origin as `via: {kind: "email"}`, a "via email" caption under the person's own bubble. The answer is an ordinary message in the app, and alerts like any other unless the origin is `quiet` (Jev judged the email only passes something on; see [notifications](notifications.md)); the Bot may also email it back (below).

**Outbound** (`sender.ts`, the email Plugin, `isolateEmail`). A Bot sends from its own address, with its name as the display name, through the deployment's sender: Cloudflare Email Service's `send_email` binding, `SEND_EMAIL`. The binding cannot be told "any address on one domain" — `allowed_sender_addresses` is a list of exact addresses — so it carries no sender restriction and `createBindingEmailSenderV1` holds the domain instead: it refuses any `from` not on `EMAIL_DOMAIN` before the binding is reached, and the kernel composes every `from` itself, never the Plugin. The kernel asks the User object what the Bot sends as (`readBotEmailSender`): the address, which needs the account's username and the Bot's switch on, and the owner's own addresses — the verified sign-in one first, then the confirmed ones. Without a username or with the switch off, nothing is claimed and nothing is sent, and the Bot is told in words it passes on. Two kinds of send, both from the seeded email Plugin (§5): mail to anyone is a `draft` card the person approves and `email_send` spends, sent with `Reply-To` the owner's sign-in address, so a reply reaches the person rather than the Bot — whose address still takes mail from its owner alone; and a note to the owner is the `owner` card, `email_owner`, which sends as it is drawn and needs no decision because the kernel holds its recipient to the owner's own addresses. That note is at most once per key — the card's surface, which the kernel mints from the Turn's tool call — claimed in the Bot object before the sender is reached (`email:owner:note:<digest>`), at most 20 per Bot per UTC day (`email:owner:count`), and given back when the send never left. When the Turn came from the owner's email, it answers that message: `In-Reply-To` and `References` are its Message-ID. It lands in the conversation as the card's receipt, "Emailed you", and in the audit trail as an `email` row, "Emailed you: <subject>"; an approved draft's send is an `email` row too.

`email.integration.ts` hands messages to the deployed Worker's own `email()` and binds `SEND_EMAIL` to a fake that keeps what it is handed: the owner's message with two files admitted once however often it arrives, a failed DMARC verdict, a stranger, an unknown username or slug, a Bot switched off and an oversized message refused, an added address confirmed by its code, a username kept to one account and every address following a change, a rename moving the address and a second Bot of the same name answering at `-2`, a deleted account's username released; and outbound, a note to the owner from the Bot's address with its receipt and audit row, a stranger's address and the day's limit refused, a Bot switched off or with no username told it cannot send, a note answering the owner's email in its thread, and an approved draft sent to someone else with `Reply-To` the owner.
...
- `#completeStep` — for a response that calls tools, the `reviewResponse` hook first, then the step's tool calls through `executeToolsV1`, then the `agent/step-continuation` waterfall and `step/end`. A step every hook would continue past still ends the Turn `completed` when `userMessageWaiting` answers true: the Bot Durable Object says so for a chat Turn on the user lane with a message queued behind it, and that message runs next ([Steering](../CONTEXT.md)).

Exhausting the loop throws `StepLimitReachedError`, which settles the Turn as `interrupted` with `STEP_LIMIT_REASON_V1`.

### Turn supervision — `app/supervision/loop.ts`

Every Turn is supervised by Jev ([plan](jev-supervision-plan.md)); the feature mounts first, so its hooks are outermost. The `request` hook asks the start-of-Turn judgment once, before the first model call, and when the person is waiting on real work appends a labeled acknowledgement note to the tail of that request — never the system prompt, which is the cached prefix. `reviewResponse` judges each response that calls tools for whether it works on what was asked; a confident wrong objective refuses every call but the Bot speaking. `prepareTool` reviews each text send right before it runs, with what the Turn already showed, for whether the person would miss it and, beside that, whether it says something was done that none of the Turn's calls did; a withheld send is never delivered and its draft is cleared. A finish withheld as redundant still ends the Turn and counts as its reply (`app/shell/delivery.ts`); one withheld for an unsupported claim, at most once per Turn, does not, and the model is told to do the thing or say it is not done. From step 5 of a long Turn, the `request` hook checks every few steps whether it is still getting anywhere: code spots the same call made again or errors in a row, and Jev judges the latest calls. A stuck Turn's request carries a tail note to change course, offering the thinking specialist when the Turn has one. Each `mutate` call — a Plugin a User installed or a Bot wrote, a remote MCP server, a connected app, as the registry resolves it into the call's context — is reviewed right before it runs for whether the person asked for it with these particulars; a refused call never runs, and the model is told to ask in conversation. When a subagent's work is in the Turn, a send that condenses or rewrites work the person asked for, other than in a way they asked, is withheld once per Turn and the Turn goes on to send it as written. A Turn opened on a subagent's `task_ask` question is told whether the conversation already answers it or only the person can. Each decision is a `supervision/*` session event, read back on resume; a withheld send becomes a `supervision` audit row. Jev gets one retry per call, then the Turn fails.
...
Validates the settled tool-occurrence journal, assembles the system prompt through `ctx.systemPrompt.assemble` (session, provider, model, turn type, subagent role, step budget, deadline), then builds one request: `session.deriveMessages()` through `agent/message-window`, `ctx.tools.schemas({turnType, subagentRole})` through `agent/tool-exposure`, and the assembled `NormalizedModelRequest` through `agent/request`. The system prompt and tool list are the cached prefix and stay the same from step to step; what changes at the end of a Turn — the step and time budget warnings (`turnBudgetHooksV1`, `app/shell/agent.ts`) — is a labeled runtime note appended at the tail of that one request, read from the agent's `turnBudget()`. Per dispatch it journals `model/request`, flushes, calls `admitEffect` — a `false` throws `EffectAdmissionFencedError` — and consumes the stream.
...
A `Task` may hand its subagent up to four of the conversation's files, named as they were attached or by upload id: the parent resolves them against its own `upload:` records before it admits the task (`findBotUploadsV1`), the task context carries the references, and the child's `subagent` Turn is admitted with them as its own message's `attachments`, read from the parent Bot's prefix like any other.
...
A Bot's look can change with nobody asking in that client: a Plugin that wraps `theme/assemble` writes its own storage, which owes an assemble at once rather than on the hour (`app/theme/owed.ts`), or another device saves one. When the look record changes, the Bot's state channel publishes a payload-free `look` update, and a client re-reads `/api/bots/:id/look` and adopts it into its directory row. Only a socket that negotiated protocol 4 is sent it; an older one's cursor moves past it unsent, because it decodes frames against a schema without that kind.
...
- `send_to_user` requires `disposition: "finish" | "continue"`. Each call delivers one separate message. Use `continue` when there is more to say or do, including another part of the answer, and `finish` on the last message to end the Turn. A `continue` send leaves a final reply owed. Widgets, approvals and secret requests always end the Turn. The Shell derives completion from the durable tool input and matching send occurrence, including after eviction.
- A `secret-request` send may carry `origin` (the site the secret is for, reduced to its origin) and `payment` beside its words in the tool input. They are the request's terms: the send tool takes them off before the payload is decoded (`splitSecretRequestTermsV1`), so the log keeps the three-field payload every installed app decodes with exact keys, and the card draw carries them to the request the kernel records.
...
- An `agent` Turn — a question handed to a Bot by the voice session or by another Bot — is a working Turn, so every work tool and model capability admits `chat`, `agent`, `automation` and `subagent`: connected apps, web fetch and search, image generation, Routines, the machine registry, the Computer, and the provider Capabilities. A question asked out loud is therefore answered from the same mailbox or calendar a typed one would be. The narrower admissions are deliberate and are about what the tool touches, not about how much the Turn is trusted: `machine-control` reaches into the person's own machine and wants them present to answer the approval card, and `bot-messaging` acts out of the thread the person is reading, so those two stay `chat` only; subagent dispatch and lifecycle stay `chat` and `automation`, which is what bounds the tree to depth one.
...
A publish never runs anything. `plugin_publish` and `plugin_enable` write a Plugin intent (`app/plugins/approval.ts`, `plugin:intent:<approvalId>`, keyed by the Turn's `effectId`) and then put an approval card on the Turn's own log naming the Plugin's tools, hooks, grants, hosts and model providers. A new publish card's rationale ends with the advisory authoring check (`app/plugins/authoring-check.ts`: the SDK lint `plugin_check` also returns, and Jev's fit judgment in `app/supervision/plugin-fit.ts`), which never blocks the publish; a replayed card is not checked again. The approval settlement (`app/approvals/bot.ts`) settles the intent in the same transaction as the decision; after the commit an approved `publish` proposes the generation on the User — the current one with the member replaced or appended, skipped when the Composition already holds that artifact, retried on a lost pin race — and switches the Plugin on for this Bot; an approved `enable` only switches. Last, any decision a person made — approved or denied, on any approval card — opens an input-delivery Turn, so the Bot says the Plugin is ready (or acts on whatever it asked for) without the person having to speak first.
...
The first seeded Plugin is `email`, `default-off`: a `draft` card over `{to, cc?, subject, inReplyTo?, body}` drawn from the standard catalog's `TextField` and the Frock catalog's `StatusPill`, `KeyValueRows`, `CollapsibleText`, `ApprovalActions` and `Receipt` — To, Cc, Subject and the body are fields the person edits, bound into a data model the surface sends back, while the thread it answers is a row they cannot change and a body too long for the Card's 16,000-byte data model is shown whole in a `CollapsibleText` and sent as drawn — so the card declares no action of its own; its `revise` turns the person's fields back into a draft, holds it to the same checks the Bot's draft met, stores it, and answers it as the decision's new `covers` — two tools the Bot spends the decision with — `email_send` and `email_discard`, each naming the card's `surfaceId` — an `owner` card, `email_owner` over `{subject, body, to?}`, which sends a note to the Bot's own person as it is drawn and is only ever a receipt ("Emailed you"), and the Skill that says which to use. Sending is not the card's: on the Turn after the person approved, the Bot calls `email_send`, which sends through `ctx.email` — the kernel loopback `isolateEmail` (`app/isolates/bot.ts`) over the deployment's own sender (`app/email/sender.ts`, `EMAIL_SENDER`), from the Bot's own address (§3, "By email"), never a credential the Plugin holds. The request names the card's `surfaceId` as well as the `approvalId`, and the loopback refuses unless the Approval is the one bound to that Plugin and surface _and_ the message digests to the values the binding recorded, so an approved decision about anything else authorizes nothing; an address that is not an address, or is longer than the kernel's own bound, is refused when the card is drawn, and a refused request answers with its own reason rather than the deployment's — and the card settles into a `Receipt` the next time it is drawn. The sender is Cloudflare Email Service's structured `send`: one call is one message to every recipient, composed by the platform, with `In-Reply-To`/`References` the only headers a draft reaches. A failure the platform names as a refusal before sending (`E_SENDER_NOT_VERIFIED`, `E_DAILY_LIMIT_EXCEEDED` and the rest of `REFUSED_BEFORE_SENDING_V1`) is `unavailable` and releases the decision's claim, so the Bot may try again once the reason is fixed; anything else — `E_DELIVERY_FAILED`, `E_INTERNAL_SERVER_ERROR`, an error with no code, an RPC whose answer was lost on the way back to the Plugin — is `unknown`: the claim stays spent, the Plugin records the mail as possibly sent, never sends it again, and settles the card into a "May have sent" receipt. A deployment that has bound no sender answers unavailable, which the Plugin tells the Bot in so many words and the Bot tells the person; the sender is bound by a deployment profile's `email` (`scripts/deployment-config/README.md`, "Email", which is also what the Cloudflare account needs first). Because `http` now opens two members rather than one, both are said wherever the reach is: the approval a User activates a Plugin with (`pluginApprovalActionV1`) and the Plugins page row (`pluginNetworkCopyV1`) name the deployment's sender beside the declared hosts, and a Plugin that declares no host of its own reads as reaching none rather than as reaching nothing — the same approval also names each card's tool, so a Plugin whose whole Bot-facing surface is cards does not read as offering nothing.
...
**Triggers (ADR 0026 step 8).** A Routine's trigger is `{ kind: "webhook" }`, `{ kind: "plugin", pluginId, trigger }`, or `{ kind: "connection", connectionId, triggerType, config? }` (`app/routines/records.ts`). Webhook and Plugin triggers enter through the same signed webhook door (`app/routines/hook.ts`, `app/routines/backend.ts`), keyed and replay-guarded alike, and `routine_manage` takes a `pluginTrigger` beside `trigger`. For a Plugin trigger the store (`RoutineStore.deliverHook`) makes the door's checks in one transaction, hands the delivery — body and the sender's headers, never the door's credential — to the Plugin outside any transaction, then enqueues the firing with the Plugin's text as the delivered payload, or records a drop with the Plugin's reason so a replay answers the same. `deliverPluginTriggerV1` (`app/plugins/triggers-bot.ts`) mounts the User's Plugin worker for the delivery alone as a registered standalone call (run `trigger:<routineId>` on the Bot's session and current generation, so its grants are admitted), delivers to `receiveTrigger`, and disposes; a Plugin that is off for this Bot, declares no such trigger, or does not mount is a drop with that reason. A device module's event (ADR 0037) enters after the door instead: the desktop posts it to `POST /api/machines/:id/module-events`, the User Durable Object admits it against the active generation and a replay guard keyed by machine, Plugin and the event's key (`app/plugins/module-events.ts`), finds the listening Routines in its index of Plugin-triggered Routines (each Routine syncs its entry when it changes; a missing index is rebuilt from every Bot once), and hands each to its Bot's `RoutineStore.deliverModuleEvent`, which runs the same replay guard, Plugin and firing with `delivery.source` naming the module. A connection trigger lives at the provider; see §8.
...
- `BotSettingsView` — `lib/settings/bot_settings.dart`: what a Bot _is_ — its
  ...
  the door to its Computer's settings where there is one, and the two danger
  rows — written
...
dispose. Audit, Look, Voice and framed WebViews stay out of that
...
`ComputerSettingsPage` (`lib/computer/settings.dart`), behind the Computer row
in Bot settings, is Save a checkpoint, Reset and Update: it says first that the
Computer is every Bot's, and Reset and Update are each a row and then a dialog
that says what is kept and what is lost before the command is sent.
...
**A Bot tries a page before publishing it** (ADR 0036 step 10). `plugin_page_try` (`computer/agent.ts`) asks the Plugin authoring host's `pageToTry` for the page built from source exactly as `plugin_publish` would store it, bridge and all, and runs it on the Bot's own Computer — never the person's shared browser. The runner (Node and `playwright-core`, launching its own headless Chromium) and the stand-in host live beside the bridge in `core/contracts/plugin-page-try.ts` and travel with each try, so nothing is installed on the Computer and they always match the release. The page is framed as the web app frames it, with no network. The stand-in greets it, answers its tool calls from the Bot's `toolAnswers`, refuses the microphone to a Plugin without the ability, and streams a scripted tone. Steps, waiting and screenshots are bounded by the `PLUGIN_PAGE_TRY_*` constants there; screenshots are small JPEGs, fetched one exec each, and filed in the `screenshots` root like `computer_screenshot`. The browser suite runs the same runner and stand-in (`apps/cloudflare/e2e/plugin-page-try.e2e.ts`).
...
One interface, `ComputerHostV1` (`computer/core/host.ts`), and two implementations of it: `computer/fly`, which is what production runs, and `computer/fake`, an in-memory host the tests substitute for it. A host `open`s a session for one Bot tenant of one User; the session is where every operation lives — `workspace`, `sync`, `exec`, `browser`, `screenshot`, `processes`, `doctor`, `presence`, `viewer`, `control`, `machine` and `logins` — and `close()` ends the session rather than the Computer. `machine` is the Computer as one machine rather than one tenant's use of it: `checkpoint`, `reset` to the newest checkpoint the host recorded, and `replace` with a fresh machine. `logins` carries the browser's sign-ins off the machine as one document only the host reads, and puts them back. `teardown?(identity)` destroys the Computer itself — its files and browser logins — and the next `open` provisions a new, empty one. Both hosts implement it, idempotently, and only the User asks for it: "Delete my Computer" (`UserConfiguration.deleteComputer`, receipted per command so a retried press never destroys a newer Computer) and deleting the account (§2). Nothing reaps a Computer on a schedule. A teardown outranks an Update or a Reset: neither brings back a Computer, or the sign-ins, that the User deleted ("Update, Reset and the User's sign-ins" below).
...
`computer/host-contract.test.ts` is one suite run twice — over `computer/fake` and over `computer/fly` on its wire-level double (`computer/fly/host-double.ts`). Its cases are the interface's own behaviour: a session opened and closed, a Workspace round-trip, the shape of an exec result, PNG bytes from a capture, a secret typed only on its origin and never answered back, a viewer opened, renewed and revoked on a URL within the host's declared origins, a control lease taken, refused to a second owner and given back, a checkpoint recorded once per effect and reset to, a machine replaced by one with none of the old one's checkpoints, the browser's sign-ins carried off and put back, an idempotent `teardown` where a host offers one, and a session that outlived a teardown refused `not-found` by `replace`, `reset`, `checkpoint` and both `logins` calls, with the next open a machine with no checkpoints. A third host is a third entry in its `HOSTS` list and no new assertion. `apps/cloudflare/test/computer-compatibility-worker.ts` is the other half: the Fly implementation in workerd, over the real v1 wire on a service binding.
...
#### Checkpoints, Reset and Update

A checkpoint is a Sprite checkpoint of the whole machine, recorded with the comment `frockbot:<effectId>`. The comment is what makes a retried `create` answer the checkpoint the first attempt made, and what a `restore` selects on: only a checkpoint this host recorded is ever restored, never one the platform took on its own. A `create` with `maxAgeSeconds` answers the newest instead when it is young enough. `restore` and `replace` are refused while a human-control lease is fresh, since both take the desktop out from under whoever holds it, and while the machine is being set up or updated; nothing opens the machine while either runs. `replace` deletes the Sprite and waits, boundedly, until its name stops answering; the next `open` provisions a fresh one under the same name in the configured region. It is refused `not-found` when there is no Sprite to delete, and when a `teardown` landed while it ran (the container counts teardowns per User), so no caller takes a replacement as leave to open a new Computer. The SDK exposes no checkpoint deletion, so checkpoints accumulate (known issue 56).

#### Sign-ins

`logins` reads and writes cookies over CDP through `browser.mjs` — `Storage.getCookies` and `Storage.setCookies` — rather than copying the profile's `Cookies` and `Login Data` files. They leave decrypted and the receiving browser encrypts them under its own key, so nothing depends on the password store either browser used (the launcher passes no `--password-store`; on this desktop, with no keyring daemon, Chromium falls back to its basic store) and nothing reads a database the running browser holds open. A capture of a machine with no browser running answers no document rather than an empty one; a restore starts the screen and the browser first. The document crosses the service binding in the clear, as file bytes do, and the host keeps none of it. Saved passwords, history and open tabs are not carried.

### Update, Reset and the User's sign-ins

The Bot Durable Object's Computer Contribution (`computer/bot.ts`) runs three commands for them. `saveCheckpoint` is answered at once. `updateComputer` and `resetComputer` are scheduled like `connect` — accepted, then run from the Bot's alarm, because the new machine takes minutes — and each is a connect with the whole machine changed in front of it: keep the sign-ins, record that the next machine is owed them, `reset` or `replace`, bring the machine up with a viewer, put the sign-ins back, and, after an Update, record the fresh machine's first checkpoint. The pending record's `machineAt` marks the one step that cannot be repeated, so a replay after eviction resumes instead of discarding the machine it already started. A connect asked for during an Update joins it; an Update or a Reset is refused while another start is pending rather than folded into it. The card reads the four steps as an `update`, and the projection's optional `checkpoint` is the newest this Bot recorded or restored.

The sign-ins are kept by the host application, never by the Computer Package (`computer/upkeep.ts` drives a `ComputerLoginVaultV1`; `app/shell/computer-logins.ts` is both halves of it). A Bot's object gzips the capture and seals it with `CREDENTIAL_KEYRING`, bound to the User and the capture time, and hands the User's object only the envelope; a capture that would not fit one Durable Object value is not kept. They are captured when a human hands the desktop back (`releaseControl`), at the end of a Turn that drove the browser at most every ten minutes, and just before an Update or a Reset. The debt an Update or a Reset records blocks every keep until it is paid — the browser of a machine that has not had its sign-ins back has none, and keeping that capture would throw the real ones away — and is paid by the restore at the end of the Update or Reset, or, when that did not happen, by the next connect or the first Computer tool call of any of the User's Bots' Turns. A Turn that used the Computer also asks for a checkpoint at most daily, and the host records one when the newest is a week old. None of this reaches the Workspace, a Session event or a model request.

"Delete my Computer" outranks an Update and a Reset, and nothing either does brings back a Computer, or sign-ins, the User deleted. `UserConfiguration.deleteComputer` forgets the kept capture and the debt, recording `deletedAt`, both before the host's `teardown` — so an Update under way sees the deletion before it can open a new machine — and after it, so a capture a Turn's end kept in between is dropped too. The vault is the fence: an Update or a Reset admitted before `deletedAt` is refused before it captures anything or changes the machine (`owe` answers `deleted`, and it checks `deletedSince` before the capture too), one under way checks `deletedSince` again before the connect that would provision a machine, and no capture made before `deletedAt` is kept. The host refuses underneath it: a `replace` with no machine, or one a teardown overtook, is `not-found`, and a replacement refused that way leaves the debt standing rather than settling it, since the machine that held the sign-ins is gone. What remains is a connect already past its last check when the teardown lands: it provisions an empty machine, which is what the next Bot to use the Computer would have done, and puts no sign-ins in it, because the debt it would pay was forgotten. A deployment with no keyring keeps no sign-ins and has no vault, so there only the host's refusals hold. Deleting the account deletes every Bot before the Computer's step, so no Update is left to race it, and wipes the User object, sign-ins included, at its end; from the moment it begins, the User object keeps no capture and records no debt.

### The in-memory host

`computer/fake/host.ts`, exported as `@frockbot/computer/fake`: one `FakeComputerV1` per User holding a `Map`-backed `FakeWorkspace`, a table of scripted `exec` answers, a one-page browser with two fields, a 1×1 PNG, viewer sessions on `https://viewer.invalid/session/<id>`, process records, a doctor report, leases keyed by scope, checkpoints that hold the sign-ins they caught, sign-ins as a list of names, and a `teardown` that destroys the Computer and can be asked twice. It records every operation it was asked for, so a suite asserts on calls rather than on a filesystem. There is no Sprite, slot, VNC or shell script anywhere in it — rule 4 is what keeps it that way.

### Protocol

`computer/host-protocol/protocol.ts` — HTTP POST per operation with optional NDJSON streaming; not a WebSocket. The envelope is `{version: 1, effectId, identity: {userId}, tenant: {botId}, credentialRef}`, and `credentialRef` is `computer:user:<userId>`, decoded and length-limited but never read (known issue 26). Fifteen operation kinds: `open`, `exec`, `file/read`, `file/write`, `file/list`, `file/stat`, `file/delete`, `control`, `viewer`, `service`, `cancel`, `teardown`, and the machine-wide `checkpoint` (`create` | `restore`), `replace` and `logins` (`capture` | `restore`). A `teardown` answers to no Bot — its envelope's tenant is `teardown` and the host ignores it — derives the Computer's name from the User, deletes it (a Computer already gone answers `deleted: false`) and forgets what the container had cached about it. None of `checkpoint`, `replace` or `logins` ever provisions a machine: each refuses `not-found` when there is none, so a `replace` after a `teardown` is refused rather than answered, and one a `teardown` overtook is refused too. Each is billed like the rest, by its active time under the host's own bound. An `open` answers an opaque `instanceId`; nothing above the host reads a host's naming out of it. Frames: `open` yields `progress | result | error`; `exec` yields `stdout | stderr | exit | error`. Requests shard to `computer-host-<fnv1a(userId) % 2>` (`apps/computer-host/src/router.ts`) and carry `x-frockbot-host-token`, checked in both the Worker and the container.
...
While a person holds the `desktop-gui` lease they can press Record in the full-window viewer, and the Bot learns from what they do (parity row 54). The session's `demonstration` capability (`ComputerDemonstrationCapabilityV1`, `computer/core/host.ts`) has two operations: `start({ownerId, seconds})`, refused unless that owner holds the lease, and `stop()`, which ends a running recording and hands back the capture once — steps and up to four screenshots — or nothing. What a step may say is fixed there too: `decodeComputerDemonstrationStepV1` is exact-field and rebuilds each step, so a step naming a field says its role, label and selector and never a value, a key is a special key or a Control/Meta shortcut and never a printable character, and a URL keeps its query's names and drops their values.

On Fly the recorder is `demonstrate.mjs`, a runtime file (`computer/fly/demonstration.ts`, installed from `runtime.ts`; the container copies and strips it beside `shell.ts`). `start` runs one unguarded `exec` that checks the lease file names the owner and is fresh, ensures the Bot's window, and launches the recorder detached; the recorder attaches over CDP to every tab of the Bot's own window, exposes a binding and injects a page script that reports clicks, which field was typed into, choices, special keys and a tab coming to the front — reading only attributes the page's author wrote, never a field's value or text, and nothing at all from a password, one-time-code or card-number field. It photographs the visible tab after navigations and clicks with every form field and frame masked, keeps at most twelve candidates spread across the recording, and stops itself when the lease is no longer the owner's, at ten minutes, or at 200 steps. `stop` signals it, runs its `collect`, and removes its directory. Both run on the host surface rather than a ready tenant, because readying asserts the agent fence a human holding control has closed; both are ordinary `exec`s, so billing meters them as any other.

In the Bot Durable Object (`computer/bot.ts`) three Computer commands drive it — `startDemonstration`, `stopDemonstration`, `discardDemonstration` — and one record, `computer:demonstrations:v1`, holds at most eight demonstrations: `recording`, `ready` or `sent`. A refused recording command settles a rejected receipt and moves no phase; the client shows its reason. Releasing control collects a running recording, and the alarm collects one whose lease lapsed or whose time ran out. A collected capture becomes files (`computer/demonstration.ts`): `demonstration-<id>.json`, the step log rebuilt field by field with a sentence saying what was not recorded, and one JPEG per screenshot, kept through `keepBotFilesV1` (`app/uploads/bot.ts`) exactly as an attached file is — counted against the account, written under the Bot's prefix with the log's text beside it, and recorded as an `upload:` the Turn command resolves. The projection's `demonstration` shows the newest one still recording or ready; the client sends a ready one as an ordinary message, "Learn this: …", carrying its files, and the Bot object marks it `sent` when a command carrying them is resolved. Pressing Record again replaces one never sent. A sent demonstration is deleted by the Bot's `demonstration_delete` once the Skill it drafted is saved or turned down (the managed `write-skill` Skill's `demonstration.md` says how: draft, ask on an `approval` card, `skill_write` at `user` scope, then delete), by Discard before it is sent, or by the alarm a week after it stopped; deleting removes the files, their `upload:` records and their quota (`releaseUploadQuota` on the User object).
...
### Saved secrets — `app/secrets/`

A Bot asks for a password, card number or other secret with a `send_to_user` `secret-request`, and the person types it into the card's host-drawn `SecretField` (§6). The value goes from the client to the Bot Durable Object's save route, which checks it against the request its draw recorded and hands it to the User Durable Object to seal as a credential generation under the `secrets` Package (§2). A Luhn-valid card number is marked a payment detail whatever the Bot asked for. The Bot is told the reference (`secret-…`), the name and the terms, on an input-delivery Turn, and nothing else.

`computer_browser` `{"action":"fill","label":…,"secret":"secret-…"}` fills one by reference. The Computer Package asks the Bot's authority first (`ComputerSecretFillSeamV1`, implemented by `app/secrets/fill.ts`): a secret that is not a payment detail — by what was saved or by a field label like "Card number" or "CVC" — fills on the one origin it was saved for without asking; anything else needs a fresh Approval of the page's origin, the field and the secret. That Approval follows the machine command's pattern: a `secret-fill:<approvalId>` intent is recorded first, an approval send (`secret-fill-` ids, reserved to the kernel) goes on the Turn's log, the Turn ends, and the Bot repeats the fill with `approval` once the person has answered. An approved fill is claimed once (`secret-fill-used:<approvalId>`) before anything is typed and must be used within ten minutes. Only then is the value leased for that one action, opened in the Bot Durable Object with the keyring, handed to the host as `ComputerBrowserAction` `fill-secret` with the one origin the page must be on, and the lease settled whatever happened. The tool result says whether the field was filled and nothing else, and a failure's words are scrubbed of the value.

On Fly the value travels as the helper's own stdin: the host writes it after the exec's script, the script's prelude reads from `/dev/null`, and its last line `exec`s `browser.mjs`, which takes the rest of stdin, so the value is never in an environment, a command line or anything bash reads as a command. `browser.mjs` checks the page's origin, fills the one field with that label in the page or one of its frames, draws it as dots (`-webkit-text-security`), marks it, and answers without a snapshot. Every later snapshot leaves out the values of marked fields, password fields and `cc-*` autocomplete fields. A refusal is told in fixed words chosen from the helper's structured answer. The in-memory host carries a one-page browser that keeps the same rule, and the contract suite holds both hosts to it. Settings lists and deletes saved secrets (`apps/native/lib/secrets/page.dart`). None of this keeps the value from code the Bot deliberately runs on its Computer, which has sudo and the shared browser's debugging port on localhost (known issue 52). Known issues 52–54 say what this does not cover.
...
Secrets are declared in `apps/cloudflare/src/production-secrets.ts`, where a required secret may belong to one auth Package: the hosted build requires `BETTER_AUTH_*` and `GOOGLE_*`, and an Access build requires `ACCESS_TEAM_DOMAIN` and `ACCESS_AUD` instead. Required of the hosted build: `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `SPRITES_TOKEN`, `COMPUTER_HOST_TOKEN`, `CREDENTIAL_KEYRING`, `ROUTINE_HOOK_SECRET`, `MACHINE_TOKEN_SECRET`, `APPLET_BUILD_TOKEN`, `OPENAI_API_KEY` (composer dictation), `GEMINI_API_KEY` (the voice session), `JEV_API_KEY` (Turn supervision, which every Turn runs under; also the routine-event rejector, group replies and dictation tidy review). Optional: `FROCKBOT_ADMIN_EMAILS`, `DEBUG_TOKEN`, `COMPOSIO_API_KEY` (Connected apps, §8), `COMPOSIO_WEBHOOK_SECRET` (Connected-app Routine events, §8), `BRAVE_SEARCH_API_KEY` (web search, §7), `FROCK_AI_GATEWAY_TOKEN`. `VOICE_ASSISTANT_MODEL` and `VOICE_DICTATION_CLEANUP_MODEL` are optional vars; `VOICE_DICTATION_UPSTREAM_URL`, `VOICE_ASSISTANT_UPSTREAM_URL` and `JEV_BASE_URL` are harness-only doors the release gate refuses to find live.
...
`mac-v*.*.*`. `qualify` runs on macOS: the machine device test files,
`scripts/mac-release-test.py`, `scripts/mac-appcast-test.py`, the Flutter
desktop update test, and an unsigned `scripts/mac-release.py` build. On a tag,
