...
What the Bot is given: `Subject: …`, then the body with a reply's quoted history cut at the line that introduces it and a signature cut at `-- ` or a phone's footer (a forward is kept whole; an HTML-only body is read in one linear pass, since it is parsed before the sender is known), bounded to 16,000 characters and 30,000 bytes, and up to five files through the same store as the composer's uploads (`storeUploadV1`, `apps/cloudflare/src/uploads.ts`: the same types, 20 MB each, the account's quota). Attachments come first, then pictures placed in the body; an inline part under 20 KB is a signature's logo and is skipped. A file refused, or past the fifth, is named in a `Not attached:` line so the Bot can say so. The Bot admits it with `admitEmailTurn`: an ordinary `chat` Turn on the user lane in the Bot's own Session with origin `{kind: "email", messageId}` and run id `em-<sha256(recipient, Message-ID)>`, so a redelivery replays the admission; the files are refs to uploads the handler recorded on that Bot first. A full queue and a Bot archived or deleted in between are refused; a failure to reach durable state throws, and Email Routing delivers the message again. The thread projects the origin as `via: {kind: "email"}`, a "via email" caption under the person's own bubble. The answer is an ordinary message in the app, and alerts like any other; the Bot may also email it back (below).
