...
45. **Voice is still almost entirely unexercised against real microphones and clients.** `docs/voice.md` and `docs/voice-live-checklist.md` say exactly what was verified: the Worker paths, the ledger, the sleep/wake ordering and the gateway are covered with fakes. Two things have run against a real provider, both on 2026-09-11 with synthesized speech rather than a microphone (`docs/voice.md`, "The live endpoint"): the dictation relay against OpenAI, and the continuous session's listening against the same realtime endpoint, driven both directly and through the real Durable Object under `wrangler dev`. What remains unobserved is the whole ElevenLabs synthesis path, the model that tidies a dictated transcript (`docs/voice.md`, "Tidying the capture"), every real microphone, and every client — no voice path of any kind has been exercised from the browser or the Flutter app. Both keys are required production secrets; a release stops until they exist.
