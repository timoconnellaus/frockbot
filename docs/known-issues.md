...
45. **Voice is still almost entirely unexercised against real microphones and clients.** `docs/voice.md` and `docs/voice-live-checklist.md` say exactly what was verified: the Worker paths, the ledger, the sleep/wake ordering and the gateway are covered with fakes. Two things have run against a real provider, both on 2026-09-11 with synthesized speech rather than a microphone (`docs/voice.md`, "The live endpoint"): the dictation relay against OpenAI, and the continuous session's listening against the same realtime endpoint, driven both directly and through the real Durable Object under `wrangler dev`. What remains unobserved is the whole ElevenLabs synthesis path, every real microphone, and every client — no voice path of any kind has been exercised from the browser or the Flutter app. Both keys are required production secrets; a release stops until they exist.

46. **The Flutter voice player pulls the macOS project onto CocoaPods and the Android build onto a compileSdk hook.** `flutter_pcm_sound` 3.3.3 ships a podspec but no Swift package, so adding it created `apps/native/macos/Podfile` and changed the Xcode project of a deliberately SPM-only app (Flutter warns this fallback "will become an error in a future version"); it also compiles against SDK 33 while pulling AndroidX libraries that need 34, so `android/build.gradle.kts` now lifts every Android subproject's `compileSdk` to 36 by reflection. Both costs sit behind the `VoicePlayer` interface in `apps/native/lib/voice/player.dart`, the only file that imports the plugin, and its `interrupt()` is `release()` + `setup()` because the plugin has no clear or pause. Swapping the player is small; carrying it is a dated dependency.
