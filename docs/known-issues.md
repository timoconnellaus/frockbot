...
47. **An MCP server's sign-in has three honest gaps.** A refresh that fails for any reason — a transient outage at the authorization server included — asks the person to sign in again, because a refresh token that rotates cannot be retried without risking the grant (`app/mcp/user.ts`, `freshen`). The public callback trusts its signed state rather than the browser that started the sign-in, as the Connect return does: someone who finishes another person's sign-in link grants that person their server account. And deleting a FrockBot account does not yet revoke its servers' grants upstream; the account-deletion saga should, from each `mcp:grant:v1:` record before the User object is wiped.
