...
| 57i | Send pipeline: a user message sent mid-Turn supersedes the running Turn — it is interrupted, a new Turn starts with the new message on the `user` lane, and background subagents keep running | `interrupt("superseded by a new user message")` + `enqueueExclusiveRun({lane:"user"})`; `interruptAll()` only for agent deletion; `unfinishedTasksReminderPending` | §B (send pipeline) | `plugin-shell` + kernel Turn admission (ADR 0022)                                                                                                                                                                                                                                           | landed                                              |
