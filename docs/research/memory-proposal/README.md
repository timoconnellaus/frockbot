...
The subsequent product decision is to replace Projects with **Group Chats**, shown in the sidebar alongside other chats with multiple member avatars. The diagrams still use the current code's Project terminology. The [Group Chat plan](../../plan.md#planned-group-chats-replace-projects) records the agreed text design; Group Chats in voice are designed later.
