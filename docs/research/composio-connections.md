...
## Google Calendar proof (2026-09-05)

The current [Google Calendar toolkit schema](https://docs.composio.dev/toolkits/googlecalendar) advertises managed OAuth and dated version `20260826_00`. `GOOGLECALENDAR_EVENTS_LIST` accepts `calendarId` (optional, default `primary`), time bounds, timezone, ordering and pagination. The provider declares its nested result data as untyped; the harness's synthetic event list follows [Google's Events list response](https://developers.google.com/workspace/calendar/api/v3/reference/events/list), not a captured production response.

`GOOGLECALENDAR_EVENT_STARTING_SOON_TRIGGER` is a polling trigger. Its current optional configuration fields are `calendarId`, `countdownWindowMinutes`, `includeAllDay`, `interval`, and `minutesBeforeStart`, with defaults `primary`, `60`, `false`, `2`, and `10`. Its event payload uses snake_case; `calendar_id`, `event_id`, and `countdown_window_minutes` are required. The older CLI marketing example uses snake_case configuration and is superseded by the current toolkit schema. Fetch schemas and pin the returned dated version; do not translate these field names by intuition.

[Calendar operator guidance](https://docs.composio.dev/kb/guide/toolkits-google-calendar) specifies `primary`, never `me`, and requires appropriate Calendar event scopes. Managed OAuth needs no operator-created Google client. A custom OAuth configuration requires a Google project with Calendar API enabled and any applicable scope verification; that setup is an operator task, not a Bot capability.
