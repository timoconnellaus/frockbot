...
| Resolved model binding     | `GET /api/settings` → `platformModel: { connectionId: "flock-ai-ambient", providerModelId: "@frock/auto" }`; `packages[]` has `custom-models` in state `disabled`; the `flock-ai-ambient` Connection is `state: ready` with `modelCatalog.state: fresh` and no key material — **pass** |
...
subtitle does **not** read `Auto (recommended) · Frock AI`. It reads
**Model unavailable**, on a client whose `/api/settings` resolves `@frock/auto`
