...
DeepSeek is served by an installed Plugin rather than a compiled adapter ([ADR 0032](adr/0032-plugin-model-providers.md)). Open the Marketplace's **Plugins** tab and choose **Add Plugin** before setting up DeepSeek in Models. Installation is account-wide but does not choose a model or create a credential; **Set up in Models** opens the separate flow for adding the API key and choosing a DeepSeek model. The key is an ordinary account Connection held server-side, and the Plugin never sees it. **Remove** in the Marketplace uninstalls the Plugin and returns a Bot that was using it to the platform default (Frock AI); the model line reports the chosen model as unavailable. Everything else in the catalog keeps its compiled adapter.
