...
- Bot isolate bindings derive from the User's enabled set. All Bots of one User under one artifact set now share a binding digest and therefore an isolate, which improves isolate reuse and removes a per-Bot cache dimension.
...
- This is a pre-user system, so the removal is a deletion rather than a migration. No compatibility path for existing Assignment records is built.
