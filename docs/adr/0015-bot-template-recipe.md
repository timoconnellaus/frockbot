...
Two smaller rules follow from the same reasoning and are recorded here rather than in their own ADRs. An uploaded avatar never travels: the bytes are keyed per-User (`botAvatarObjectKeyV1`), copying them forks user-scoped content, and the image may be a person's face — so a template carries the Bot's own generated `SheepRecipeV1`, which is four layer ids and nobody's photograph. And a template document carries no `createdAt`: the bytes must be a pure function of the Bot they describe, so that re-exporting an unchanged Bot lands on the same key and writes nothing. When a share was packed is share metadata, and it lives on the share record, which is mutable state anyway.
