...
- _8b, done._ Source off the Computer. `applet_publish` lists the Applet's source prefix in the Workspace store, posts it to `APPLET_BUILD` under the Turn's effect id, verifies the returned hashes against the bytes and stores the artifacts as before — the pre-publish pull off the Sprite and the last import from `computer/` into the Applet path are deleted. The Bot edits with `applet_files`, `applet_read_file` and `applet_write_file`, and `applet_check` builds without publishing: it answers with the diagnostics, or with the declared tools and a preview URL on the anonymous artifact origin. The four shell steps are gone from the tool text and the Skill, and `APPLET_BUILD_TOKEN` is now a required production secret.
