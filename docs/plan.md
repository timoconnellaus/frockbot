...
- _6c, in progress._ Frock Compose hosts the untrusted layer only. The Package Catalog went first: first-party code ships with the deploy, so it had nothing to distribute. Then the Applets tools became an ordinary first-party feature with first-party iframe pages, and the Worker Loader host moved from `kernel-composition` into `compose-frockbot`, mechanics unchanged. What remains: the manifests become static definitions for first-party code and Frock Compose descriptors — the six named actions, the slots, the grant list and the three context keys — for untrusted members, generations and activation move into `kernel-do`, and `kernel-composition` goes. Nothing produces an untrusted member until the step 8 build service exists, so the narrowed contract lands with tests as its only caller.
