...
Conversation summaries run on `@frock/structured`, the `frock-structured` gateway route, for every Bot whatever its own model; they are billed as hosted calls, so that id needs its own entry, sized for a summariser request of about 100k bytes. The gateway holds each request to its own model's bounds. Do not guess a price for the gateway's automatic route. Pin its concrete model, verify its provider prices, and configure the matching Frock AI model entry before launch. Hosted image-containing chat requests are refused until their token bounds can be quoted safely; BYO models continue to support them through their provider.
