...
Hosted image generation has no configured unit-price contract yet and refuses without charging. This is a visible unavailable feature, not a blocker for subscriptions, text models, BYO models or Computer usage. It must receive its own bounded reservation and durable operation key before being offered to billed accounts. Memory embeddings and ordinary application storage/hosting are product overhead covered by the subscription; they are not silently added to a customer's model bill. Jev decisions are product overhead too, at US$0.042 per million input tokens: Turn supervision and every other platform judge, and the calls a Plugin makes through its `jev` grant. A Plugin's are itemised under it on the Work view, with no cost, and capped at 64 a run.
...
`GET /api/billing` answers with available monthly, purchased and complimentary credit, pending reservations, the subscription and its period end, the plan, recent usage, recent credit grants and what the account spent in the last 30 days. Usage pagination uses SQLite insertion sequence, so equal timestamps do not skip rows.

## The Billing page

The Flutter app has one **Billing** page, a row on You. It opens on what the account can spend: one total, a pill saying the subscription renews (or ends) on its period end, and the parts of that total — monthly credit against the plan's allowance and when it resets, top-up credit, which never expires, and complimentary credit only while there is some. A subscribed account adds credit there (one of the plan's top-up amounts, then Checkout), and opens its plan, invoices and card in the Stripe portal. One line under it says how long the credit lasts at the last week's pace, that hosted models and Computers draw on it with no overage charge, and what is held for work still running. An account whose Bots cannot reply — not subscribed and out of complimentary credit, subscribed with nothing left, or suspended — is told so first, in red; an account without a subscription is shown the plan, its numbers from `plan`, with Subscribe. Below that is Spending, then a Prices row that opens the Computer rate and the hosted model rates.

## Spending

**Spending** answers where an account's credit went. It is a section of Billing, which search can open scrolled to it. A Bot's settings and a Routine's run log open the same view on a page of its own, narrowed to that Bot or Routine and showing the credit left beside what it cost. It leads with the answer: what the period cost against the period before, how long the credit lasts at the last week's pace and whether that is past its renewal (on its own page; in Billing the balance says it), and the cause that drove most of it. Under that, a bar per day split by the groups the breakdown lists, in the same colours, and the breakdown itself by what started it, Bot, what it bought or model; tapping a row narrows the page to it, and each narrowing is a chip that widens again. The most expensive Turns sit beside the breakdown on a wide screen and under it on a phone, each opening its Bot's conversation. `GET /api/billing/spending` also serves the trigger, conversation and Plugin dimensions, which the page does not offer.

The dimensions are the Bot that spent it; **what started it** — a chat with a Bot, a Routine, a Group Chat, voice, mail to a Bot, the person using a Computer directly, or a Plugin's own page; the trigger (the person, email, a schedule, a webhook, a connected app, run by hand); the conversation; what it bought (model replies, conversation summaries, web search, Computer time); the model; and the Plugin that made the call.
...
A person can give any Bot or Routine a **daily limit** from its row under Spending (`POST /api/billing/limits` with a scope of `bot|<botId>` or `routine|<botId>|<routineId>` and `dailyMicros`, or `null` to remove it). A day is the person's own, from midnight in their Profile timezone.

A limit stops background work, never the person: a charge whose cause is a chat, a Group Chat, voice, the person on the Computer or a Plugin's own page is never refused by one, and a Bot's limit counts only its background spend, so the person's own chatting cannot use it up. Every other attributed charge is held to its Bot's limit, and a Routine's charges — in whichever Bot its chain reaches — to the Routine's. A day starts at the person's midnight, daylight saving included; where it starts on the half hour, the hourly rollup rounds it up, so the first minutes go uncounted rather than yesterday's last ones holding a limit past midnight. The check is inside the reservation's own transaction and on what has settled today, so the charge that crosses a limit completes and the next one is refused with `DAILY_LIMIT_REASON_V1`, which a failed Turn shows as written. A charge already reserved is never refused again: its retry is the same charge. An unattributed charge is held to nothing.

Before each firing, a Routine asks whether its own or its Bot's limit is reached. The first firing a limit stops in the person's day — before it runs, or part way through — fails with a message in the Bot's conversation, the same message and push as any Routine that did not run. Only once that message is written does the Routine note the day, and the rest of that day's firings are skipped quietly, so a Routine that runs every half hour says so once. An account with no limits answers the check without reading anything else.

After a firing that finishes, the Routine asks for a **spike**: a day at least three times its average over up to the previous seven days it has existed, and at least US$0.50. The account hands the alert to one firing a day, which posts it on its own run as a message — "has spent US$0.91 today, about 4× its usual US$0.22 a day" — through the same path. A Routine with fewer than three days of history has no usual yet and raises none.
