...
an account switch. Native account-scoped local storage and browser account-scoped local
...
| Integrations and secrets                  | Provider validation/OAuth and credential storage remain backend implementations. Dart settings imports only Flutter/UI, typed transport, local command storage and the system URL launcher. Its only credential-related value is a scoped navigation URL from the backend.                                                                                                                 |
...
| Landing and rollout                       | Required checks, release screenshots and the dated acceptance ledger gate the C1 PR. Pixel evidence awaits C1 deployment; the phone was unlocked after restart. The orchestrator owns release tags; production macOS verified sign-in remains gated by provisioning.                                                                                                                       |
