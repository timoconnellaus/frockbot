...
public final class FrockBotMobilePlugin extends Plugin {
    ...
    public void invoke(PluginCall call) {
        if (!exact(call.getData(), Set.of("schemaVersion", "commandId", "input")) || call.getInt("schemaVersion", 0) != 1) {
    ...
    private static boolean exact(JSObject actual, Set<String> expected) {
    ...
    private void showNotification(PluginCall call, JSObject input) {
        if (!exact(input, Set.of("title", "body", "urgency")) && !exact(input, Set.of("title", "urgency"))) {
