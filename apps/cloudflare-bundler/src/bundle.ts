/**
 * The Package bundler seam, kept out of the Worker entry module on purpose.
 *
 * workerd reads every named export of `main` as an entrypoint, so a plain
 * `export const` there makes the Worker refuse to start: "Incorrect type for
 * map entry 'BUNDLER_VERSION': the provided value is not of type 'function or
