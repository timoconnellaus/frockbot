...
enum VoiceModeState { listening, speaking, paused, failed, ended }
...
VoiceModeState voiceModeStateOf(AssistantSessionController session) {
  ...
  // A call the server closed first is over in the same way, having failed at
  // nothing: the word stops claiming a live call without wearing the error.
  if (session.endedLine != null) return VoiceModeState.ended;
...
String voiceModeWordOf(VoiceModeState state) => switch (state) {
  ...
  VoiceModeState.ended => 'Call ended',
};
...
class _VoiceModeState extends State<VoiceMode> {
  ...
  _Presentation _read() => (
    ...
    // A call that failed or ended, or is saying something short-lived, is the
    // whole reason this surface exists at that moment: the footer that used
    // to carry both is not drawn while voice mode is (ADR 0029).
    text:
  ...
  ///
  /// The slot is also where the call speaks: a failure that ended it, the end
  /// itself, or a notice that borrows the stage for a few seconds, is the same
  /// shape of thing — one line about the call, where the call's other output
  /// goes. Which of them is showing is decided by precedence, exactly as the
  /// footer decides it, so the two surfaces never disagree about what is being
  /// said.
