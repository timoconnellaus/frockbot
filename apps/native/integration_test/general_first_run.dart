...
Future<String> signIn(NativeApi api, PlainStore store) async {
  ...
  try {
    final response = await client.send(
      http.Request('GET', Uri.parse(start['authorizationUrl'] as String))
        ..followRedirects = false,
    );
    expect(response.statusCode, 302);
    await response.stream.drain<void>();
    expect(
      await signIn.accept(Uri.parse(response.headers['location']!)),
      isTrue,
    );
  } finally {
    client.close();
  }
  return (jsonDecode((await store.read('session'))!) as Map)['userId']
      as String;
}
...
void main() {
  ...
  if (origin.scheme != 'http' ||
      ...
      statePath == null ||
      !Directory(statePath).existsSync()) {
  ...
  testWidgets('native sign-in opens General; pending Bot links take precedence', (
    ...
    Future<PlainStore> newStore(String name) async {
      final store = PlainStore(
        location: () async => Directory(statePath),
        name: name,
      );
      await store.load();
    ...
    final generalId =
        (await api.request('/api/bots/bootstrap') as Map)['generalBotId']
            as String;
    ...
    await until(
      tester,
      () => find.widgetWithText(AppBar, 'General').evaluate().isNotEmpty,
    );
    expect(await store.read('selection.$userId'), generalId);
    expect(find.byType(BottomSheet), findsNothing);
    final starter = find.byWidgetPredicate(
      (widget) =>
          widget is Semantics &&
          widget.properties.identifier == StarterIds.suggestion('project'),
    );
    ...
    expect(
      ...
      'Plan my garden project',
    );
    expect(
      (await api.request('/api/bots/$generalId/turns') as Map)['runs'],
      isEmpty,
    );
    debugPrint(
      'NATIVE_ACCEPTANCE: real native sign-in opened General; starter is an unsent draft',
    );
    ...
    for (final pendingAtMount in [true, false]) {
      ...
      await until(
        tester,
        () => find.widgetWithText(AppBar, 'Rosemary').evaluate().isNotEmpty,
      );
      ...
      debugPrint(
        'NATIVE_ACCEPTANCE: pending link ${pendingAtMount ? 'at mount' : 'during loading'} opened Rosemary before General',
      );
      await tester.pumpWidget(const SizedBox());
