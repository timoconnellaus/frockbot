...
The foot of the Profile page shows the running version, with the booted Shorebird patch number beside it on Android. A build from a version tag shows the tag, from `--dart-define=FROCKBOT_RELEASE=<version>`: `release.yml` passes it to the web, macOS and Android builds, and a patch carries its own tag, so the number matches the GitHub release whose code is running. `pubspec.yaml`'s version names the native build instead. It moves only when the native client must, because the compatibility gate and the Shorebird release identity key on it. Any other build shows that native build, from `--dart-define=FROCKBOT_APP_VERSION=<name>+<code>`, which `scripts/native-update.py`, `scripts/native-desktop-update.py` and the web build pass from `pubspec.yaml`. A plain `flutter run` carries neither and shows "Development build".
