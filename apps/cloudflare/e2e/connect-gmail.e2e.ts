...
for (const [connector, search] of [
  ["Gmail", "gmail"],
  ["Google Calendar", "calendar"],
]) {
  test(`${connector} is a direct searchable connector with durable connected and disconnected states`, async ({
    page,
    userId,
  }) => {
    ...
    await page
      .getByRole("menuitem", { name: "Connectors", exact: true })
      .click();
    await page.getByRole("searchbox").fill(search!);
    const card = page
      .locator(".connector-card")
      .filter({ has: page.getByText(connector!, { exact: true }) });
    ...
    await page.screenshot({
      path: `e2e/test-results/${search}-connected.png`,
      fullPage: true,
      animations: "disabled",
    });
