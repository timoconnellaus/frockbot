...
class MemoryStorage {
  ...
  put(key: string | Record<string, unknown>, value?: unknown): Promise<void> {
    ...
    else for (const [entry, stored] of Object.entries(key)) this.values.set(entry, stored);
...
describe("push device registry", () => {
  test("strictly decodes registrations before they reach the user registry", () => {
    expect(decodePushRegistration({ deviceId: "phone-1", token: TOKEN_A, activeBotId: "primary" }))
  ...
  test("token refresh replaces one installation and removal deletes it", async () => {
    ...
    await registerPushDevice(durable, { deviceId: "phone-1", token: TOKEN_A }, 100);
    await registerPushDevice(durable, { deviceId: "phone-1", token: TOKEN_B, activeBotId: "primary" }, 200);

    expect(await durable.list({ prefix: "push:device:" })).toEqual(new Map([
      ["push:device:phone-1", { deviceId: "phone-1", token: TOKEN_B, activeBotId: "primary", updatedAt: 200 }],
    ]));
    await registerPushDevice(durable, { deviceId: "phone-1", remove: true }, 300);
    expect(await durable.list({ prefix: "push:device:" })).toEqual(new Map());
  });
});
...
describe("push dispatch", () => {
  ...
  test("one focused device defers the alert, then an expired lease delivers it", async () => {
    const durable = storage();
    const now = Date.now();
    await registerPushDevice(durable, { deviceId: "desktop-1", activeBotId: "primary" }, now);
    await registerPushDevice(durable, { deviceId: "phone-1", token: TOKEN_A }, now);
    const sends: Array<{ token: string; notify: boolean; data: Record<string, string> }> = [];

    const sender = async (_secret: string, token: string, data: Record<string, string>, notify: boolean) => {
      sends.push({ token, notify, data });
      return "sent" as const;
    };

    await expect(deliverPush(durable, "user-1", message(), SECRET, sender))
    ...
    expect(sends).toEqual([{ token: TOKEN_A, notify: true, data: expect.objectContaining({ notify: "true" }) }]);
  });

  test("expired presence and focus on another Bot do not suppress an alert", async () => {
    const durable = storage();
    const now = Date.now();
    await registerPushDevice(durable, { deviceId: "old-view", activeBotId: "primary" }, now - 15_001);
    await registerPushDevice(durable, { deviceId: "other-view", activeBotId: "other" }, now);
    await registerPushDevice(durable, { deviceId: "phone-1", token: TOKEN_A }, now);
    const notified: boolean[] = [];

    await deliverPush(durable, "user-1", message(), SECRET, async (_secret, _token, _data, notify) => {
  ...
  test("a duplicate dispatch does not send a message/device delivery twice", async () => {
    const durable = storage();
    await registerPushDevice(durable, { deviceId: "phone-1", token: TOKEN_A }, Date.now());
    let sends = 0;
    const sender = async () => { sends += 1; return "sent" as const; };
  ...
  test("an uncertain external send is recorded and never blindly repeated", async () => {
    ...
    expect(await durable.get("push:delivery:primary:message:phone-1"))
      .toMatchObject({ cursor: "message-00000000000000000001", status: "uncertain" });
  });
  ...
  test("an older concurrent send cannot overwrite the newer delivery receipt", async () => {
    ...
    const sender = async (_secret: string, _token: string, data: Record<string, string>) => {
  ...
  test("an unregistered token removes the installation after the claimed attempt", async () => {
    const durable = storage();
    await registerPushDevice(durable, { deviceId: "phone-1", token: TOKEN_A }, Date.now());
    await deliverPush(durable, "user-1", message(), SECRET, async () => "unregistered");
