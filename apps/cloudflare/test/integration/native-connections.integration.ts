...
test("browser and native see the same secret-free Connectors home", async () => {
  ...
  expect(view).toMatchObject({ schemaVersion: 1, ownerId: userId, accounts: [] });
  const native = await SELF.fetch("https://bot.frockbot.com/api/settings/connections", { headers });
