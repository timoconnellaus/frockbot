...
- A branch need not be rebased when `main` moves: the merge queue reruns `Check` on the combination before it lands. Rebase only to resolve a conflict, and then inspect the rebased diff rather than rerunning no-mistakes because the commits moved.
...
- Merging integrates; tagging ships. A green `main` deploys staging and cuts the next patch tag by itself; production moves only when Tim approves the release run under the `production` environment. Never approve a deployment or push a version tag unless asked.
