...
- A Composition generation is keyed by its resolved artifact set, so identical artifacts share one loader identity. Generation creation rate, artifact size, retained generations, Workspace disk, and Bot isolate CPU, subrequests, and model spend are bounded by durable per-User quotas; exceeding a quota refuses the operation and records a visible failure.
...
- Memory is Markdown files under a durable root of the Workspace, in a Bot Memory root per Bot and a User Memory root shared by the User's Bots. A Bot reads Memory with ordinary file tools on the Computer or through the Memory Package; it changes Memory only through the Memory Package, which functions while the Computer is hibernated.
