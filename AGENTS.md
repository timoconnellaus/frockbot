...
- **Untrusted code gets an isolate.** Plugins and Applets run in a loaded Worker with `globalOutbound` disabled and only its named grants, reached through loopback bindings whose props the Bot Durable Object mints per Bot per Turn. No ambient network, no secrets, no Durable Object storage it was not given. Plugins in one User's worker share a realm, so a grant one holds is honestly described as held by all of them.
