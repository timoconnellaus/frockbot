...
- Once no-mistakes finishes, go directly to push, PR and CI. Do not rerun the full local test suites after the gate; the PR's CI is the next validation layer. Rerun locally only when code changes after the gate or when the gate skipped or failed a required suite.
- A branch may complete no-mistakes before its release slot. If the base moves afterward, do not rerun no-mistakes solely because the commits moved: inspect the rebased diff, run the required tests once, then proceed to merge.
