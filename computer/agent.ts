...
export function createComputerAgentFeature(
  ...
  return (runtime) => {
    ...
    const browserTool: ToolDefinition = {
      ...
      idempotent: config.idempotentEffects === true,
      description:
        'Control the browser in the Bot\'s selected Computer and return an accessibility snapshot. Shapes: {"action":"snapshot"}; {"action":"navigate","url":...}; {"action":"click","role":"button","name":"Add"} (role AND name, both from the snapshot line, e.g. checkbox "Mark done"); {"action":"fill","label":"New todo","text":...}; {"action":"press","key":"Enter"}; {"action":"wait","milliseconds":500}. For a password, card number or other secret the user saved, fill by its reference instead of text: {"action":"fill","label":"Password","secret":"secret-…"}. You never see the value; the result says only whether the field was filled. A payment detail, or a secret used on a site other than its own, first asks the user to approve that page and field; once they have, repeat the same fill with "approval" set to the id you were given.',
      inputSchema: {
        type: "object",
        properties: {
          action: {
