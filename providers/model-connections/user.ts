...
export interface ModelConnectionClientConfigV1 {
  apiBaseUrl?: string;
}

export interface ModelConnectionClientV1 {
  listModels(apiKey: string): Promise<ConnectionModelV1[]>;
  resolveModel(apiKey: string, id: string): Promise<ConnectionModelV1>;
  probeInference(apiKey: string, modelId: string): Promise<void>;
}

export interface ModelConnectionLifecycleConfigV1 {
  packageId: string;
  connectionTypeId: string;
  displayName: string;
  /** Durable decode failures use this adapter-owned name. */
  storageLabel?: string;
  /** Product name used in Connection command failures. */
  connectionLabel?: string;
  authorizationKind?: "api-key" | "grant";
  providerType: string;
  ...
  preferredProbeModelId?: string;
  createClient(config: ModelConnectionClientConfigV1): ModelConnectionClientV1;
  validateSettings?(settings: Record<string, string>): void;
}

type ModelConnectionCredentialContribution = Omit<
  CredentialUserBackendContribution,
...
export interface ModelConnectionUserBackendHostV1 {
  ...
  credentials: ModelConnectionCredentialContribution;
  /** A client the host supplies for every Connection; wins when given. */
  client?: ModelConnectionClientV1;
  /** A test or embedding host may override the adapter's client factory. */
  createClient?(config: ModelConnectionClientConfigV1): ModelConnectionClientV1;
  beforeModelLease?(input: {
    accountId: string;
    connectionId: string;
...
/** Each keyed provider owns an independent instance of the same durable connection lifecycle. */
export function modelConnectionLifecycleV1(
  config: ModelConnectionLifecycleConfigV1,
) {
  const DISPLAY_NAME = config.displayName;
  const STORAGE_LABEL = config.storageLabel ?? DISPLAY_NAME;
  const CONNECTION_LABEL = config.connectionLabel ?? DISPLAY_NAME;
  const PACKAGE_ID = config.packageId;
  const CONNECTION_TYPE_ID = config.connectionTypeId;
  const COMMAND_PREFIX = `${config.storagePrefix}-connection-command:`;
  const PENDING_KEY = `${config.storagePrefix}-pending-connection-commands`;
  const RECEIPT_INDEX_KEY = `${config.storagePrefix}-connection-receipt-index`;
  const COMMAND_TOMBSTONES_KEY = `${config.storagePrefix}-connection-command-tombstones`;
  ...
  const ACCOUNT_KEY = `${config.storagePrefix}-connection-account`;
  const AUTOMATIC_REFRESH_RECEIPT_PREFIX = `${config.storagePrefix}-refresh-receipt:`;

  const MUTATION_SEQUENCE_PREFIX = `${config.storagePrefix}-mutation-sequence:`;
  const MODEL_RESOLUTION_PREFIX = `${config.storagePrefix}-model-resolution:`;
  const REFRESH_INTERVAL_MS = 60 * 60 * 1_000;
  /**
  ...
  const CATALOG_RETRY_BASE_MS = 60_000;
  const MAX_CATALOG_RETRY_ATTEMPTS = 8;
  const CATALOG_RETRY_PREFIX = `${config.storagePrefix}-catalog-retry:`;
  ...
  interface StoredCommand {
    ...
    expectedGeneration?: string;
    /** A keyed model Connection never admits the keyless create command. */
    operation: Exclude<
      ConnectionCommandV1["type"],
      "connection/create" | "connection/oauth"
    >;
    label?: string;
  ...
  function probeModelId(models: readonly ConnectionModelV1[]): string {
    const preferred = config.preferredProbeModelId
      ? models.find(
          (model) => model.providerModelId === config.preferredProbeModelId,
        )
      : undefined;
    const chosen = preferred ?? models[0];
    if (!chosen) {
      throw new Error(
        `${DISPLAY_NAME} exposed no model to validate the key against`,
      );
    }
   ...
   */
  function decodeConnectionSettings(
    input: ConnectionSettingsV1 | undefined,
  ): Record<string, string> {
    const entries = Object.entries(input ?? {});
    if (entries.length > MAX_CONNECTION_SETTINGS_V1) {
      throw new Error(`${DISPLAY_NAME} Connection settings are too many`);
    }
    const accepted: Record<string, string> = {};
    for (const [key, value] of entries) {
      if (
        key !== config.apiBaseUrl?.settingKey &&
        !config.settingKeys?.includes(key)
      ) {
        throw new Error(
          `${DISPLAY_NAME} Connection setting "${key}" is not supported`,
        );
      }
      if (
        typeof value !== "string" ||
        !value.trim() ||
        value.length > MAX_CONNECTION_SETTING_VALUE
      ) {
        throw new Error(`Connection setting "${key}" is invalid`);
      }
      accepted[key] =
        key === config.apiBaseUrl?.settingKey
          ? config.apiBaseUrl.decode(value)
          : value.trim();
    }
    config.validateSettings?.(accepted);
    return accepted;
  }
  ...
  function decodeStoredConnectionSettings(
    ...
    if (!input || typeof input !== "object" || Array.isArray(input)) {
      throw new Error(
        `Stored ${STORAGE_LABEL} Connection settings are invalid`,
      );
    }
    ...
    try {
      return decodeConnectionSettings(raw);
    } catch {
      throw new Error(
        `Stored ${STORAGE_LABEL} Connection settings are invalid`,
      );
  ...
  function connectionApiBaseUrl(
    ...
    const candidate = connection?.settings?.[endpoint.settingKey];
    return candidate === undefined ? undefined : endpoint.decode(candidate);
  }
  ...
  function decodeStoredAccount(input: unknown): string {
    return storedText(input, `Stored ${STORAGE_LABEL} account`, 256);
  }

  function decodeCommandIdList(input: unknown, label: string): string[] {
    if (!Array.isArray(input)) {
      throw new Error(`Stored ${STORAGE_LABEL} ${label} is invalid`);
    }
    return [
  ...
  function decodePendingCommands(input: unknown): string[] {
    ...
    if (pending.length > MAX_PENDING_COMMANDS) {
      throw new Error(`Stored ${STORAGE_LABEL} pending commands are invalid`);
    }
    return pending;
  }
  ...
  function decodeReceiptIndex(input: unknown): string[] {
    ...
    if (index.length > MAX_MANUAL_RECEIPTS) {
      throw new Error(`Stored ${STORAGE_LABEL} receipt index is invalid`);
    }
    return index;
  }
  ...
  function decodeCommandTombstones(input: unknown): StoredCommandTombstone[] {
    if (!Array.isArray(input) || input.length > MAX_COMMAND_TOMBSTONES) {
      throw new Error(`Stored ${STORAGE_LABEL} command tombstones are invalid`);
    }
    return input.map((value) => {
      const command = decodeStoredCommand(value);
      if (!command.receipt || command.completedAt === undefined) {
        throw new Error(`Stored ${STORAGE_LABEL} command tombstone is invalid`);
      }
  ...
  function decodeStoredCommand(input: unknown): StoredCommand {
    ...
    if (
      ...
      throw new Error(`Stored ${STORAGE_LABEL} command is invalid`);
    }
    ...
    if (
      ...
      throw new Error(`Stored ${STORAGE_LABEL} command receipt is invalid`);
    }
    return {
      schemaVersion: 1,
      commandId,
  ...
  function decodeStoredModelResolution(input: unknown): StoredModelResolution {
    const value = storedRecord(
      input,
      `Stored ${STORAGE_LABEL} model resolution`,
      [
        "schemaVersion",
        "effectId",
        "accountId",
    ...
    if (
      ...
      throw new Error(`Stored ${STORAGE_LABEL} model resolution is invalid`);
    }
    const model =
  ...
  function decodeMutationSequence(input: unknown): {
    ...
    const value = storedRecord(
      input,
      `Stored ${STORAGE_LABEL} mutation sequence`,
      ["schemaVersion", "next", "applied"],
    );
    if (
      value.schemaVersion !== 1 ||
      ...
      throw new Error(`Stored ${STORAGE_LABEL} mutation sequence is invalid`);
    }
  ...
  function compactCompletedCommand(
    ...
    if (!record.receipt || record.completedAt === undefined) {
      throw new Error(
        `Completed ${CONNECTION_LABEL} command receipt is unavailable`,
      );
    }
    return {
      schemaVersion: 1,
  ...
  return class ModelConnectionUserBackendContribution {
    ...
    constructor(private readonly host: ModelConnectionUserBackendHostV1) {
    ...
    async executeConnection(
      ...
      if (command.type === "connection/oauth")
        throw new Error("OAuth is not supported by this connection");
      return this.executeCommand(accountId, command, false);
    }
    ...
    private async executeCommand(
      accountId: string,
      command: Exclude<ConnectionCommandV1, { type: "connection/oauth" }>,
      automaticRefresh: boolean,
    ): Promise<ConnectionCommandReceiptV1> {
      ...
      if (storedAccount && storedAccount !== accountId) {
        throw new Error(
          `${CONNECTION_LABEL} Connection authority does not match`,
        );
      }
    ...
    private async admit(
      ...
      if (command.type === "connection/create-api-key") {
        if (
          ...
          throw new Error(`${DISPLAY_NAME} Connection type is invalid`);
        }
        if (
          !(await this.host.settings.isPackageInstalled(accountId, PACKAGE_ID))
        ) {
          throw new Error(
            `${DISPLAY_NAME} Package is not installed and enabled`,
          );
        }
        ...
        try {
          accepted = decodeConnectionSettings(command.settings);
        } catch (error) {
          settingsFailure = (
            error instanceof Error && error.message
              ? error.message
              : `${DISPLAY_NAME} Connection settings are invalid`
          ).slice(0, 500);
        }
      ...
      if (command.type === "connection/create") {
        throw new Error(`${DISPLAY_NAME} Connections require an API key`);
      }
    ...
    private async ensureAdmittedState(
      ...
      if (record.operation === "connection/create-api-key") {
        ...
        if (!current) {
          await this.host.settings.createConnection(
            record.accountId,
            this.authorizingConnection(record, record.label ?? DISPLAY_NAME),
          );
        }
      }
    ...
    private async admitRecord(
      ...
      const admit = async (transaction: CredentialTransaction) => {
        ...
        if (pending.length >= MAX_PENDING_COMMANDS) {
          throw new Error(
            `${CONNECTION_LABEL} Connection command capacity reached`,
          );
        }
        if (!record.automaticRefresh) {
          ...
          if (
            ...
            throw new Error(
              `${CONNECTION_LABEL} Connection command history capacity reached`,
            );
          }
        }
        let admitted = record;
    ...
    private authorizingConnection(
      ...
      return {
        ...
        authorization: {
          schemaVersion: 1,
          kind: config.authorizationKind ?? "api-key",
          credential: {
            schemaVersion: 1,
            configured: true,
            source: config.authorizationKind ?? "api-key",
            writable: true,
            generation: record.credentialGeneration,
            updatedAt,
          },
      ...
      if (!outcome.validationStatus) {
        try {
          if (outcome.providerRetryPolicy !== "safe-metadata-read") {
            throw new Error(
              `${CONNECTION_LABEL} validation retry policy is unavailable`,
            );
          }
          ...
          // Catalog reads are not necessarily authenticated. Each adapter's
          // probe decides what proves that the credential can perform inference.
          const client = this.clientFor(
            (config.apiBaseUrl
              ? record.settings?.[config.apiBaseUrl.settingKey]
              : undefined) ?? connectionApiBaseUrl(existingProjection),
          );
          ...
          outcome = {
            ...
            validationFailure: (error instanceof Error && error.message
              ? error.message
              : `${DISPLAY_NAME} validation failed`
            ).slice(0, 500),
        ...
        await this.host.storage.transaction(async (storage) => {
          ...
          if (storedValue === undefined) {
            throw new Error(
              `${CONNECTION_LABEL} Connection command is unavailable`,
            );
          }
          const stored = decodeStoredCommand(storedValue);
          if (!stored.receipt) {
        ...
        await this.host.storage.transaction(
          async (storage: UserSettingsTransaction & CredentialTransaction) => {
            ...
            if (!current || current.packageId !== PACKAGE_ID) {
              throw new Error(`${DISPLAY_NAME} Connection is unavailable`);
            }
            if (
              record.operation === "connection/create-api-key" &&
                  ...
                  modelCatalog: {
                    ...
                    failure:
                      outcomeFailure instanceof Error
                        ...
                        : `${DISPLAY_NAME} catalog refresh failed`,
                  },
                },
                storage,
              );
    ...
    private async cancelPendingCredentialMutations(
      ...
      const ordered = [
        ...receiptIndex.filter(
          (commandId) => !completedIds.includes(commandId),
        ),
        ...completedIds,
      ];
          ...
          if (current.state === "revoked") {
            ...
            await this.host.settings.replaceConnection(
              ...
              expectedGeneration,
              {
                ...
                failure: `${DISPLAY_NAME} does not expose upstream API-key revocation`,
              },
              storage,
            );
        ...
        async (storage: UserSettingsTransaction & CredentialTransaction) => {
          ...
          const pending = (
            pendingValue === undefined
              ? []
              : decodePendingCommands(pendingValue)
          ).filter((id) => id !== record.commandId);
          if (record.automaticRefresh) {
    ...
    private async scheduleNextAlarm(accountId: string): Promise<void> {
      ...
      const catalogDeadlines = packageInstalled
        ? settings.connections.flatMap((connection) => {
            ...
            if (
              ...
              connection.connectionTypeId !== CONNECTION_TYPE_ID ||
      ...
      if (deadlines.length > 0) {
        ...
        const existing = await this.host.storage.getAlarm?.();
    ...
    async leaseModelCredential(input: {
      ...
      await this.host.beforeModelLease?.(input);
      ...
      if (!known) {
        ...
        const resolution = await this.host.storage.transaction(
          async (storage: UserSettingsTransaction & CredentialTransaction) => {
            ...
            if (
              ...
              throw new Error(
                `Stored ${STORAGE_LABEL} model resolution authority changed`,
              );
            }
        ...
        if (journal.status === "pending") {
          if (!resolution.lease) {
            throw new Error(
              `${CONNECTION_LABEL} model resolution lease is unavailable`,
            );
          }
          ...
          try {
            ...
            outcome = {
              ...
              failure: (error instanceof Error && error.message
                ? error.message
                : `${DISPLAY_NAME} model resolution failed`
              ).slice(0, 500),
            };
          }
          journal = await this.host.storage.transaction(async (storage) => {
            ...
            if (storedValue === undefined) {
              throw new Error(
                `${CONNECTION_LABEL} model resolution journal is unavailable`,
              );
            }
    ...
    /**
     * Lease this Connection's credential for a tool effect. It is the model
     * lease without model resolution or catalog reconciliation.
     */
    ...
    private async requireModelAuthority(
      ...
      if (
        ...
        throw new Error(`${DISPLAY_NAME} Package is not installed and enabled`);
      }
    ...
    /** A test client wins; production delegates construction to the adapter. */
    private clientFor(apiBaseUrl?: string): ModelConnectionClientV1 {
      if (this.host.client) return this.host.client;
      const clientConfig: ModelConnectionClientConfigV1 =
        apiBaseUrl === undefined ? {} : { apiBaseUrl };
      return (this.host.createClient ?? config.createClient)(clientConfig);
    }
...
/** Shared User host slice for every model Connection adapter. */
export interface ModelConnectionsUserApplicationHostV1 {
  modelConnections: ModelConnectionUserBackendHostV1;
}
