...
export type ModelConnectionClientV1 = Pick<
  OllamaCloudClient,
  "listModels" | "resolveModel" | "probeInference"
>;
...
export interface OllamaUserBackendHost {
  ...
  createClient?(config: OllamaCloudClientConfig): ModelConnectionClientV1;
...
export function modelConnectionLifecycleV1(
  ...
  const DISPLAY_NAME = config.displayName ?? "Ollama Cloud";
  ...
  function decodeOllamaConnectionSettings(
    ...
    for (const [key, value] of entries) {
      if (key !== API_BASE_URL_SETTING && !config.settingKeys?.includes(key)) {
        throw new Error(
        ...
        const resolution = await this.host.storage.transaction(
          async (storage: UserSettingsTransaction & CredentialTransaction) => {
            ...
            if (
              ...
              throw new Error(
                "Stored Ollama model resolution authority changed",
              );
            }
