...
name: simplify-code
description: >-
  Parallel 3-agent cleanup of recent code changes. Use when the user says
  "simplify", "simplify my changes", "review my code", "review my recent
  changes", "clean up my changes", or "/simplify". Optional modifiers: focus
  on reuse/quality/efficiency, dry run / just report, or a scoped diff
  (last commit, staged, this branch, specific files). Do not auto-run after
  every edit.
license: MIT
---
...
Adapted from [Hermes Agent `simplify-code`](https://github.com/NousResearch/hermes-agent/blob/d05769a6bd8bed56cc071914962854384cacc004/skills/software-development/simplify-code/SKILL.md) (MIT).
...
Launch **three** `Task` subagents in **one message** (three tool calls together)
so they run concurrently. Use `subagent_type: "generalPurpose"` for each.
Three is the right fan-out; do not add more reviewers.
...
wider codebase. Each reviewer can use shell, file, and search tools (`git`,
`Read`, `Grep`).

If this repo has `AGENTS.md`, `CONTEXT.md`, or a linter config, fold the
relevant conventions into each reviewer prompt so suggestions match house
...
- Skip nits and style-only churn. Only flag things that materially improve
  the code.
- Require `file:line` evidence; a reuse finding with no pointer to an existing
  utility is noise.
...
> (use Grep) for existing functions, constants, or patterns the new code
> could call instead of reimplementing. Flag: new functions that duplicate
> existing ones; hand-rolled logic that an existing utility already does
> (manual string/path manipulation, custom env checks, ad-hoc type guards,
> re-implemented parsing). For each, name the existing thing to use and
> where it lives.
...
4. **Apply** the surviving fixes directly with `StrReplace` / `Write` — unless
...
- **Respect project conventions.** If the repo has `AGENTS.md` / `CONTEXT.md`
