...
The left sidebar lists the authenticated User's Bots and switches the conversation. **Add a Bot** creates a Bot with a character, a name and the first thing to say to it; pressing the character in Bot settings opens the picker where its character and colour change. Hidden and archived Bots each fold into their own group at the foot of the list; an archived Bot opens read-only, its conversation kept, with **Restore** and **Delete** where the composer would be. A Bot's page — its Computer, its recent Routine firings and the doors of its Plugin panels — is the right panel at wide widths and a pushed page on the phone, opened from the Bot's name in the conversation bar; its settings are one level under it behind the gear, and the Bot's Plugins — what it could run and whether it does, one switch per row, each switch that Bot's own — are a row in those settings, ending with the Bot's setup history: each version of the Plugin code it has written, and whether it ran. The **Marketplace** opens from the Bot list as one searchable catalog of models and some 1,500 connected apps, from Gmail, Google Calendar and Slack to Shopify and Stripe. Checkboxes under the search box choose Models and Connectors; Installed is the same list limited to what has already been added, for configure and remove. Adding a model provider opens its key form, and a connected provider's card offers **Choose a model**. From the profile sheet, **Settings** owns the declared application settings and **Models** holds the account's default model and the providers already added; each Bot may choose its own model in its settings, and without a choice follows that default — Frock AI's Automatic until one is chosen. Built-in features such as Web and Routines are switched per Bot on that Bot's Plugins page; there is no account-wide switch for them. During an active Turn, **Stop** records cancellation intent; closing or switching clients does not stop backend work. **Delete**, on the profile page, deletes the Computer — the next Bot that needs one gets a new, empty one — or, once the person types the address they sign in with, the whole account and everything in it, at once.
...
The phone app ships from the same run. `Ship Android` builds a signed Shorebird patch of `apps/native` against the newest active Android release and puts it on the staging track as soon as the tag verifies; `Promote Android patch` moves it to stable once `Deploy FrockBot app` has succeeded, so the client goes stable together with the server it was built against, and never ahead of it. A tag whose `apps/native` matches the previous release tag cuts nothing. When Shorebird finds native, asset or plugin changes, the same job cuts a full release with `native-update.py release` and uploads that APK. Installing it once puts the phone on the baseline later patches target (see [`apps/native/README.md`](apps/native/README.md)). Neither Android job is on the web deploy's path, so neither can hold production back.

`frockbot.apk` on the GitHub release is the sideload. A tag that cut a full release attaches the APK it just built. Any other tag's `Fetch Android APK` job downloads the newest active Shorebird release and re-signs it with the keystore below; installing that file and opening it picks up the stable patch. The jobs read `SHOREBIRD_TOKEN` and `ANDROID_DEBUG_KEYSTORE_BASE64`; the patch private key stays on the patch job.
...
app/              The product: `runtime.ts`, the Contribution tables, and one directory per feature
  ...
  connect/        Connected apps: the connection backend, its app tools, and the generated app list
