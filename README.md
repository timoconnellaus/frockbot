...
2. **Merge** — a maintainer clicks merge. The pull request enters the merge queue, which reruns `Check` against current `main` and lands it. There is no auto-merge: a green pull request waits for a person. A branch need not be rebased first; the queue proves the combination.
3. **`main`** — `main.yml` runs the slow tier once per landed change: the Cloudflare workerd and integration suites, the real build, and the browser suite across four runners. Green deploys staging, cuts the next patch tag on that revision, and starts `release.yml` for it. A push that touches only `docs/**` and root Markdown starts no run.
