...
Once a version tag verifies, `release.yml` deploys five Cloudflare Workers — marketing, the admin portal, the Applet build service, the Computer host and the app — through the GitHub `production` environment, and only then publishes the `frockbot.npm` workspaces to npm. Publishing waits on the backend deploy because a package published for a release whose deploy then failed would sit on the registry ahead of a production that never moved. Merging to `main` deploys nothing — a tag is the only thing that reaches production, so code can be integrated freely and released deliberately:
