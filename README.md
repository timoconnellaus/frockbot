...
The same Wrangler file declares Cloudflare's `AI` binding for production and development. Flock AI uses its Gateway surface, while `generate_image` uses native image inference. The binding needs no FrockBot secret, but the Cloudflare account must have billing for the configured Gateway routes and native models. The browser e2e environment binds `AI` to a local RPC fake instead, so CI neither authenticates to Cloudflare nor incurs model usage.
