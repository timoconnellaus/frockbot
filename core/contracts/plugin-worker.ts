...
/** The `<action>` half of a card action name, bounded like a surface id. */
const PLUGIN_CARD_ACTION = /^[a-zA-Z0-9][a-zA-Z0-9._-]{0,127}$/;

export function decodePluginWorkerCardActionInvocationV1(
  input: unknown,
  label = "plugin worker card action invocation",
  ...
  const action = boundedString(value.action, `${label}.action`, 128);
  if (!PLUGIN_CARD_ACTION.test(action)) {
  ...
  return {
    ...
    ...(value.context === undefined
      ? {}
      : { context: record(value.context, `${label}.context`) }),
    ...(value.dataModel === undefined
      ? {}
      : { dataModel: record(value.dataModel, `${label}.dataModel`) }),
    botId: boundedString(value.botId, `${label}.botId`, 256),
