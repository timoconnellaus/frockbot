...
**Consumer deployment**:
A second product that installs published `@frockbot/*` packages and writes only its choosers, wrangler, catalog option, and `ProductConfig`. Not a fork, not a copied tree, not a third profile in this repository. The in-repo fixture is `examples/consumer/`.
_Avoid_: White-label, fork, edition
