...
describe("message-time unread and notification records", () => {
  ...
  test("an email that only passed something on is answered without waking a device", async () => {
    const event = send(1, "Filed your receipt.");
    const answered = (quiet: boolean) => ({
      ...run([event]),
      admission: {
        schemaVersion: 1 as const,
        turnType: "chat" as const,
